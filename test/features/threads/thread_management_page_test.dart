...
void main() {
  ...
  testWidgets('主题标签完成编辑后关闭面板并更新表单', (tester) async {
    await pumpThreadManagementTestPage(
      ...
      ThreadManagementTestRepository(
        initial: threadManagementTestBootstrap(tagNames: const ['原标签']),
      ),
    );
  ...
  testWidgets('无管理权限时不渲染保存、邀请或删除入口', (tester) async {
    final repository = ThreadManagementTestRepository(
      initial: threadManagementTestBootstrap(canManage: false),
    );
    await pumpThreadManagementTestPage(tester, repository);
  ...
  testWidgets('版本冲突保留表单并允许采用云端最新版', (tester) async {
    final repository = ThreadManagementTestRepository(
      ...
      latest: threadManagementTestBootstrap(version: 3, title: '云端最新版标题'),
      conflictOnce: true,
    );
    await pumpThreadManagementTestPage(tester, repository);
    await tester.enterText(
