...
- Windows 移动端升级必须固定 Foundation `v6.7.0`，把 `align` 加入 Delta 行属性白名单，并让 Markdown→Delta Codec 只为普通段落与 H2/H3 解码、编码紧邻的 `center|right` 标记；左对齐删除标记。工具栏以单一“对齐”入口在更多面板切换左/中/右，阅读器按块应用一致排版；列表、引用、普通图片、分隔线和协议空段不得继承属性，提及、骰子和收藏表情随父段落。客户端先同时接受服务端版本 3 与 4，再将自身支持版本升级到 4；后端激活前 `/meta.markdownContractVersion` 仍为 3。
