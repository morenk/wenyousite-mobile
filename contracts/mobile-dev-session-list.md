...
- `blocked` 表示无法证明登记安全，例如损坏状态、非终态的 PID 复用、缺失或不匹配的设备锁、无法通过带控制 token 的 status 核验、存活子进程、遗留 reverse 或孤立设备锁。已完整停止、无自有锁和 reverse 且无自有子进程的历史登记，不因历史 PID 被其他进程复用而阻塞。根级 `blocked` 是任一条目 blocked 的汇总。
...
- 列表只是瞬时核验，不是授权或排他租约。治理切换入口与 Mobile start 必须持有共同的 `transition.lock`，锁协议固定为治理提交 `1338a78407ce83b9b0d039c59d411c0a67a45a39` 的 `docs/preview-session-control.md`；在同一锁内查询并完成过渡操作。不得以一次旧列表长期授权切换。旧版本 start 不认识新锁，启用治理控制前必须同步两端已审核工具。
