...
当前阶段是公网开发环境上的快速迭代，不是正式生产发布。所有公网联调必须使用专用测试账号；禁止对共享开发数据运行批量删除、账号注销或其他破坏性自动化。

## 2. 产品范围

V1 包含认证、公开浏览、搜索、主题/子贴/楼层、创作管理、图片、提及、骰子、草稿、收藏、关注、订阅、拉黑、账号设置和站内通知。
...
- 创建主题、子贴、楼层和回复的同一次用户操作必须复用同一 UUID `clientRequestId`。
- 断网时保留草稿，但不得静默自动发布。
...
5. 同步测试和模块文档。
6. 运行相关测试；切片完成后运行统一质量门禁。
7. 自查 diff、生成文件、无关修改和敏感信息。
8. 原子提交并默认推送 `dev`。
9. 等待 CI；CI 失败立即停止新功能并以独立 `fix` 恢复绿色。
10. 汇报行为、文档、测试和可观察结果。

切片完成定义：主路径可操作；加载/空/错/重试/权限状态完整；测试和文档同步；静态检查零问题；契约无漂移；无伪实现和调试残留；已推送 `dev` 且 CI 绿色。
...
- `0.3.0-dev`：首页、搜索、主题、子贴、楼层和用户主页。
- `0.4.0-dev`：回复、点赞、收藏、关注、订阅、拉黑和通知。
- `0.5.0-dev`：编辑器、图片、提及、骰子、本地和云草稿。
