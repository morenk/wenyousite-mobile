...
- 公网 API：`https://wenyou.site/api/v1`；线上自动化只读，不作为开发预览回退地址。
- Tailnet API：`https://wenyou-vps.tail3993f3.ts.net/api/v1`；同样按线上环境保护，仅用于只读兼容性检查。
- 开发预览 API：从已核验的私有 `consumer.json` 读取独立端口，真机经同端口 SSH 与 ADB reverse 访问；禁止固定使用共享 `3000`。
...
共享审美与跨端体验事实源只存在于 `wenyousite-foundation` 的远端最新正式发布版本；当前由 `pubspec.yaml` 锁定 Foundation v7.2.1。移动端仓库不维护平行审美规范，只记录模块行为与代码入口。页面必须复用 `WenyouThemeTokens`、Foundation 语义图标、全局 `ColorScheme` 与共享组件，禁止在页面内创建近似 Token 或直接使用 Material 图标。功能阶段不得顺手引入大范围插画、粒子或复杂换皮。

阅读滑块是移动端独有交互，由本仓库的 `ReadingScrollSpec`、控制器与共享阅读组件维护尺寸、唤醒阈值、时序和状态机，不再作为 Foundation 跨端契约，也不为手感微调发布 Foundation 版本。通用颜色、圆角、字体与无障碍基础样式仍复用应用主题；这项边界不改变其他跨端契约的发布依赖规则。
...
9. 汇报行为、文档、本地验证、实际执行的测试路径、Debug 会话源码摘要／必要 APK 和真机手测清单；Bug 按下述流程交给负责人复验，日常开发不等待 CI。
...
- 普通低、中风险 Bug 在精确回归、直接受影响测试、全量静态分析及对应 Debug 会话／必要 APK 验证通过后即可提供候选，不要求候选前先跑全量 Flutter 测试。候选记录必须列出实际测试文件或目录、提交号、未验证项和操作步骤，不能只写“相关测试通过”；APK 另记录版本与 SHA-256。项目负责人亲自复验原场景，涉及真机的问题在对应持续 Debug 源码摘要或独立候选安装包上验证；热重载后的画面不可用初装 APK 哈希代表。
...
- 负责人明确通过后，记录对应候选、验收日期和结果，再更新完成状态；普通低、中风险切片在合并前对最终应用源码执行一次完整门禁。验收失败时保留既有测试与安装记录，同时更新当前失败现象及下一步排查依据，后续候选只重复精确回归、直接受影响测试、全量静态分析与持续 Debug 验证；仅构建条件变化或需要独立候选时再次构建 APK，不在每轮机械重复全量门禁，也不能拿此前通过的自动检查覆盖人工失败结果。

### 开发中快速反馈循环

Flutter 本地开发中的“热重载”和“热重启”只用于 Debug 反馈，不表示向已安装客户端动态下发代码的生产热更新。用户授权当前任务后，开发代理通过 `npm run dev:start -- --session <consumer.json>` 自行识别已连接 ARM64 真机并启动本任务持久 Debug 会话；同一反馈批次跨对话复用。控制器使用 Flutter machine 协议，不接管其他任务的设备、Worktree 或 Debug 包。启动前必须核验已提交 Backend 协议与实际 PostgreSQL、Redis、媒体身份；缺失或冲突立即停止，禁止回落公网。详见 `docs/live-debug.md`。

- 颜色、间距、字号、文案、普通布局和 Widget 组合等纯 Dart 展示调整，优先执行 `npm run dev:reload` 热重载，并保留当前页面与操作状态快速观察结果。
- `main()`、初始化逻辑、Provider 装配、路由、全局或静态状态等不能由热重载可靠重新执行的变化，执行 `npm run dev:restart` 热重启并重新进入目标页面。
...
同一视觉目标的连续微调可以组成一个反馈批次，批次内不在每次保存后机械重复 analyze、测试或构建。三阶段分别承担不同证据，不能互相冒充：开发反馈使用热重载／热重启和直接相关测试；候选验证使用全量静态分析、针对性测试与持续 Debug 会话（记录 runId、设备、源码摘要和画面）；需要脱离调试会话验收或原生变化时才生成独立 Debug APK；集成验证在负责人验收通过后、合并前运行完整门禁。热重载或热重启不能替代本阶段要求的检查，候选验证也不等于每轮都要执行全量门禁。没有会话时，已授权的开发代理默认启动本任务隔离预览 Debug；环境或设备未就绪时提供 Widget/Golden 并列出未覆盖状态，不把未查看画面当作视觉验收。`dev:stop` 保留批次数据库和本地草稿，不清理其他任务的 reverse/SSH。
...
2. **候选验证**：普通低、中风险切片显式指定相关测试，通过全量静态分析后在持续 Debug 会话交由负责人真机验收；仅需独立候选时构建 APK。
3. **集成验证**：普通低、中风险候选验收通过后、合并 `dev` 前，对最终应用源码运行一次完整门禁。认证、契约、网络、上传、持久化、幂等、注销、依赖和 Android 原生配置等高风险切片仍须在提供候选前完成完整门禁和 APK 构建。

需要独立 Debug APK 的普通候选使用以下唯一快速入口，必须传入至少一个位于 `test/` 子目录内的测试文件或目录；脚本执行全仓 Dart 格式检查、应用与生成客户端全量静态分析、传入的相关测试和 Debug APK 构建，并输出 APK 绝对路径、大小与 SHA-256：
