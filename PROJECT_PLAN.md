...
- 客户端：`0.3.0-dev.77+83`，Android 8+，正式包名 `site.wenyou.app`，Debug 包名 `site.wenyou.app.debug`。
- 后端契约：`4.13.0-dev.20260815.1`，revision `8c82f3551f58b2060f602802e84893ac7b8d8b78`。
...
2. 接入动态评论通知精确定位：调用 `momentsCommentContext`，补取评论上下文、滚动到目标，并复用 1dp 淡粉边框的短时提示。
3. 接入用户主页背景图与活动汇总；在公开资料和本人编辑流程中补齐加载、权限、上传与失效状态。
...
- 动态评论通知仍只进入动态详情，尚未消费已经稳定的 `momentsCommentContext`。
- 不支持的 Markdown 结构在编辑器中以可解释源码显示，提交时会转成 Markdown v3 安全字面文本；这不是原始不支持结构的无损提交。
- 同步的 `contracts/mobile-client-guide.md` 仍未描述站内邀请引用；本地以 fixture 和代码为准，差异记录在 [上游文档漂移](docs/upstream-documentation-drift.md)。
