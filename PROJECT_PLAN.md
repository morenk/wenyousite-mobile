...
- 后端契约：`5.17.0-dev.20260905.2`，revision `80e9820b06d3f7cbea209e11d2b7be5353add0a5`；签到返回当天实际发放经验（0～2），内容互动经验由后端异步幂等结算；主题帖档案导出、子贴主楼层置顶、Markdown v5、编辑器往返黄金契约 v6、剪贴板契约 v2 与独立图片块对齐保持激活，公网 `/meta` 仍为 `5.16.0-dev.20260904.1` / `8fb7d17`，与已同步的远端契约存在部署差异，公网一致性门禁待后端部署后复核。
