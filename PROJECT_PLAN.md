...
- 客户端：`0.7.0+94` Android 正式发布；新正式 APK 仅支持 Android 8+ ARM64，正文、标题和品牌文字继承平台系统字体，KaTeX、等宽呈现与 Material Icons 等功能依赖保留。正式包名 `site.wenyou.app`，Debug 包名 `site.wenyou.app.debug`，真机性能包名 `site.wenyou.app.profile`。负责人已对正式签名包完成 ADB 覆盖安装并明确验证通过；线上 `/meta` 已晋级推荐 build 94。
...
1. `0.7.0+94` 已完成正式签名包 ADB 覆盖安装、负责人验收、RainS3 上传与线上 `/meta` 晋级；后续可从仍停留旧版本的设备补充应用内更新链路复核。
