...
- 后端契约：`5.26.0-dev.20260922.3`，来源 `92b030a81f8957386e324fed477bd1e46faf65ea`，与 2026-09-23 公网 `/meta` 一致；生成客户端已同步图集查询、本人关系列表管理和仅供管理端使用的内容整理接口。移动端尚未接入图集连续浏览及本人粉丝移除/双向关系投影，分别保留为独立功能切片。
...
- 契约 5.25 已提供本人关注/粉丝列表的双向关系投影及移除粉丝端点，现有移动端列表仍只读展示，管理操作需独立接入并验证跨页面缓存失效。
- 契约 5.26 已提供内容图片图集的双向游标查询；移动端全屏查看器当前只浏览当前页面已加载图片，连续跨页图集作为独立高风险媒体切片接入。
