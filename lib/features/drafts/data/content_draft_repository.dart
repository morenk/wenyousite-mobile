...
class ApiContentDraftRepository implements ContentDraftRepository {
  ...
  Future<ContentDraftCollection> fetchCollection() async {
    try {
      final snapshot = (await _api.draftsState()).data?.data;
      if (snapshot == null) {
        throw const ApiFailure(userMessage: '正文草稿加载失败，请稍后重试。');
      }
      final mappedDrafts =
          snapshot.drafts.map(_mapDraft).toList(growable: false)
      ...
      return ContentDraftCollection(
        ...
        usage: ContentDraftSlotUsage(
          usedSlots: snapshot.usedSlots.toInt(),
          maxSlots: snapshot.maxSlots.toInt(),
          occupiedSlots: snapshot.slots.map((slot) => slot.toInt()).toSet(),
        ),
      );
  ...
  Future<void> remove(String id, {required int version}) async {
    try {
      final data = (await _api.draftsRemove(
        id: id,
        version: version,
      )).data?.data;
