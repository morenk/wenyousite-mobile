...
组件统一第一批（2026-09-19，候选／待负责人验收）：登录终端退出和注销最后确认接入共享确认框；注销原有固定短语输入与多步骤确认保留。退出终端和继续注销使用共享异步按钮，危险语义由按钮变体表达，会话清理及仅本地清理重试不变。 入口清单、自动检查及真机步骤见[组件统一验收](../architecture/component-consistency-acceptance.md)。
