...
本模块页面排版统一遵循[移动端视觉基线](../architecture/visual-baseline.md)中的 Foundation v6.8.0 语义文字角色，不自定义字号或直接依赖 Material 字体槽位。资料编辑、账号设置、外观、故障诊断、黑名单、登录终端、修改密码、更换邮箱与注销账号统一套用设置字体作用域：保留语义字号、行高、字重和颜色，但页面及区块标题全部使用 Foundation 正文字体，不使用展示楷体；数字 utility 字体保持不变。

认证生命周期由 auth 实现，资料由 users，关系和黑名单由 social，头像由 media，治理决定和专用凭据由 moderation；设置只编排入口，不复制仓储逻辑或持有申诉凭据。全局主题只消费 Foundation v6.8.0 的亮色/黑夜 Token、主题偏好标签和语义图标；内容结构、图片资源及业务状态不因外观模式变化。
