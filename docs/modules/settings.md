...
承载全局外观偏好、账号安全、终端会话、资料与头像隐私编排、黑名单及治理决定入口、关于和开发环境信息。当前已交付“跟随系统 / 亮色 / 黑夜”外观切换、“我的”资料/头像管理、修改密码、更换邮箱、安全退出、账号注销、登录终端管理、黑名单与治理决定/申诉入口；V1 不含实验室或远程配置控制台。

## 2. 用户角色与使用场景

游客可从“我的”切换外观或进入登录；登录用户还可管理头像、资料隐私、本人关系列表、黑名单、密码、邮箱、活跃登录终端、退出和注销，关于与开发信息后续接入。外观偏好属于设备本地设置，不要求登录，也不随账号切换。

## 3. 页面、入口和导航关系

Android 账号设置帮助组新增公开“更新说明”入口，进入应用壳的已发布版本历史；iOS 与不支持的平台不展示该入口。版本读取、分页与更新资格由 app-shell 维护，不依赖账号资料，也不产生升级后弹窗。本项为候选／待负责人验收。

改密、换邮箱与登录终端沿用个人区柔和页底和统一内容宽度；改密与换邮箱表单移除重复小标题，输入约束、退出影响、验证码、危险确认和失败恢复仍保留。加载失败操作统一显示“重试”。
...
普通登录用户还可从账号设置进入公开 `/appeals` 治理决定页；设置只提供入口，不读取或持有 moderation 的专用申诉凭据。账号设置页面只依赖当前认证会话即可展示安全、关系与退出入口，不等待本人资料请求，因此资料加载失败不会阻断设置。

2026-09-25 个人区视觉候选／待负责人验收：账号设置依次展示“偏好与提醒 / 账号 / 账号操作 / 帮助”四组；外观仍进入独立页，不增加版本或其他新入口。外观、诊断、资料编辑及账号设置复用 Foundation 柔和浅色页底、白色分组面板、组内细线和语义图标；组内设置项标题为常规字重，后台消息提醒不显示右侧说明按钮，标题长按及读屏仍可获得耗电和停止条件说明；黑夜继续使用既有暗色 Token。退出和注销保留原确认、失败恢复与权限边界；诊断、通知申请及外观保存行为不变。
...
设计统一后三批候选／待负责人验收：账号设置、外观和故障诊断共用 WenyouSettingsBody 的内容宽度、响应式边距、安全区与底部留白。外观、省流量和后台提醒的读取/保存失败共用 WenyouSettingsFailure。省流量保存失败保留本次运行选择；后台提醒保存失败回滚，重试恢复失败那次目标；读取失败重试不写入。诊断存储失败只提示复制恢复，不提供无效重试。登录终端继续展示精确时分。 交付及手测见[三批统一验收](../architecture/design-unification-completion.md)。
...
组件统一第一批（2026-09-19，负责人验收通过）：登录终端退出和注销最后确认接入共享确认框；注销原有固定短语输入与多步骤确认保留。退出终端和继续注销使用共享异步按钮，危险语义由按钮变体表达，会话清理及仅本地清理重试不变。 入口清单、自动检查及真机步骤见[组件统一验收](../architecture/component-consistency-acceptance.md)。
...
通知引导（2026-09-13，负责人验收通过）：首次登录的前台会话通过应用壳的一次性提示引导授权，点击才申请；拒绝后不自动重试。账号设置在通知不可用时提供“申请权限”，系统不再询问或频道关闭时可用“系统消息通知设置”手动开启。新消息默认使用系统通知音；横幅/声音由用户在“新消息提醒”中调整，常驻通知保持静音。旧频道若已被设为“无”，升级保留原选择，不覆盖为有声。
...
外观页新增“省流量”开关，开启后帖子列表封面保持静态。默认关闭且允许所有网络；用户切换立即生效，写入失败保留本次选择、显示失败说明并允许重试，不因持久化失败重新下载动画。读取失败时保守开启省流量，允许重新读取。
...
`/appearance` 是公开路由且不读取账号资料；`/me` 只在认证状态请求私有资料。`/me/following`、`/me/followers`、`/me/blocks` 与五个 `/me/security/*` 路径都受路由鉴权，游客先登录并保留安全回跳。用户名、邮箱冲突和限流以服务端为准；当前密码错误按业务码 `40116` 显示稳定提示。终端界面只根据 `platform` 映射平台文案，禁止读取或展示已废弃的 `deviceInfo` 原始 UA；其他页面只显示脱敏或用户正在换绑的邮箱，不展示 Token、密码、验证码、敏感凭据或预签名 URL。注销确认短语只存在表单内存；页面明确服务端已发布内容匿名保留和本地草稿不会由注销端点删除。
...
省流量以 SharedPreferences 键 `media.dataSaver.v1` 保存设备本地布尔值，缺省关闭。`main.dart` 在首帧前读取并注入 Notifier 状态；退出账号不清除设备选择，不新增账号 API。设置不清空已有动画字节缓存，关闭后仍需经过列表停稳与可见性选择才播放。

账号注销已由服务端确认后立即撤销内存凭据；若本地安全存储清理失败，路由仅保留原注销页的清理重试，不放行其他受保护页面，也不重复提交注销。该例外依赖当前页面的远端确认状态，不接受 URL 参数授权。

外观显式模式以 SharedPreferences 键 `appearance.preference.v1` 保存 `light` 或 `dark`；选择跟随系统时移除覆盖键，缺失或未知值回退跟随系统。该偏好不含隐私数据，退出、注销和切号都不清除。资料、关系列表、终端列表和表单不持久化，账号设置服务端为权威；页面销毁、切号和退出释放私有状态。终端撤销成功后仅按稳定 ID 从当前列表移除，下次进入重新读取；改密、换邮箱、当前账号退出或注销成功后均交由 `SessionController` 原子清理本地双 Token，会话状态变化同时触发首页发现列表权威刷新。注销不会静默删除账号隔离的未发布本地草稿，但原账号不可恢复，因此必须在确认前告知风险。

## 9. 加载、空数据、错误、重试和冲突状态

自动诊断 v2 候选补充安全源码栈、固定错误原因与页面/运行环境信息；关闭、清除、切号及保留期限不变，不新增主动提交样本入口。字段边界与待验收项见[故障诊断](../architecture/failure-diagnostics.md)。

公开 `/diagnostics` 故障诊断页供游客从“我的”、登录用户从“账号设置”进入，可控制自动发送、逐条复制或清除本机记录。最多保留 7 天内的 50 条记录，1 MiB 上限；关闭开关停止待发送，退出或切号清除记录。页面不需要读取账号资料。详见[故障诊断](../architecture/failure-diagnostics.md)。
...
外观偏好启动读取失败时首帧安全使用跟随系统，并在外观页提示重新读取；写入期间禁用重复选择，写入失败回滚旧模式、保留目标模式并允许重试。表单提交防重复，未变化不发请求；失败保留输入、旧资料与问题编号。黑名单与终端读取均以列表结构 Skeleton 起始，并提供空、失败、重试和刷新；写入串行执行。终端撤销确认期间禁止重复点击；最新列表仍包含目标或读取失败时显示中性提示并允许刷新，不重复撤销。终端和邮箱验证码限流使用全局 `42900` 友好提示，邮箱验证码额外按 `Retry-After` 倒计时；未知平台降级为“其他终端登录”，任一账号安全成功响应缺失 data 时不伪装成功。已有简介因当前 DTO 不支持 null 而不能清空，客户端明确提示。当前账号退出失败保留会话并提供本机后备。注销失败保留短语、会话和问题编号；远端成功后的本地清理失败只能重试本机清理。

## 10. 跨模块约束

本模块页面排版统一遵循[移动端视觉基线](../architecture/visual-baseline.md)中的 Foundation v6.9.0 语义文字角色，不自定义字号或直接依赖 Material 字体槽位。资料编辑、账号设置、外观、故障诊断、黑名单、登录终端、修改密码、更换邮箱与注销账号统一套用设置字体作用域：保留语义字号、行高和颜色，页面、区块及子区块标题使用 Foundation 正文字体 600，不使用展示楷体；数字 utility 字体保持不变。

认证生命周期由 auth 实现，资料由 users，关系和黑名单由 social，头像由 media，治理决定和专用凭据由 moderation；设置只编排入口，不复制仓储逻辑或持有申诉凭据。全局主题只消费 Foundation v6.9.0 的亮色/黑夜 Token、主题偏好标签和语义图标；内容结构、图片资源及业务状态不因外观模式变化。

## 11. 测试场景与验收条件

- [ ] 后台消息提醒开关、系统频道入口、常驻卡片与 30 分钟双消息横幅场景的负责人真机复验，见[候选验收记录](../architecture/background-reminders-acceptance.md)。

- [x] 游客看到登录入口且不请求私有资料；登录用户的账号设置不依赖本人资料加载，资料失败时仍可管理安全、黑名单、申诉、退出与注销。
...
- [x] 所有设置与账号配置页面通过同一字体作用域使用 Foundation 正文字体，页面、区块和子区块标题不再出现展示楷体，浅色/黑夜 Golden 保持覆盖。
...
2026-09-26 个人区视觉整合复核：随 `dev` 固定已部署 Backend `124fb4e8aa395440f7a2156de98b642ec87f7583`、OpenAPI `5.26.0-dev.20260922.3` 与最新正式 Foundation `v7.1.2`；外观页保留“省流量”入口，本轮无 HTTP API 或存储变更。
