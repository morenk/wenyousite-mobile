...
认证生命周期由 auth 实现，资料由 users，关系和黑名单由 social，头像由 media，治理决定和专用凭据由 moderation；设置只编排入口，不复制仓储逻辑或持有申诉凭据。全局主题只消费 Foundation v6.5.1 的亮色/黑夜 Token、主题偏好标签和语义图标；内容结构、图片资源及业务状态不因外观模式变化。
