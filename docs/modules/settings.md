...
设置文案精简候选：后台提醒只保留常驻、耗电及划掉停止的必要说明，系统通知设置副标题为“管理消息弹窗”；未授权与设置打开失败使用简短操作提示，不解释系统默认提示音或轮询周期。实际频道、授权与后台行为不变。

通知引导候选（2026-09-13，待负责人验收）：首次登录的前台会话通过应用壳的一次性提示引导授权，点击才申请；拒绝后不自动重试。账号设置在通知不可用时提供“申请权限”，系统不再询问或频道关闭时可用“系统消息通知设置”手动开启。新消息默认使用系统通知音；横幅/声音由用户在“新消息提醒”中调整，常驻通知保持静音。旧频道若已被设为“无”，升级保留原选择，不覆盖为有声。
