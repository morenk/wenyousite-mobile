...
选择子贴后沿不透明 cursor 加载楼层；接近末尾或点击按钮继续分页。独立回复页先校验主楼层，再按最早/最新排序和回复者筛选读取回复，首屏外的精确目标会额外读取并注入正确位置。登录用户从主题详情发表楼层，从独立页回复原楼层或指定回复；作者可打开同一 Quill 编辑器修改内容，管理者可在主题详情删除楼层。管理者在子贴正文卡片添加或编辑 BODY 帖。所有已有主题帖子编辑器输入 `@` 均以当前 `threadId` 查询候选并插入规范提及节点。
