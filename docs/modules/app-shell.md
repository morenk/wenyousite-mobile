...
- [x] 冷启动或回前台首帧稳定后发现新的完整楼层/回复与私密邀请链接时先询问再进入准确路由；恢复瞬间暂不可读仍能在帧后识别，拒绝、同一内容、普通主题、混合文字和站外链接不重复或误触发。
