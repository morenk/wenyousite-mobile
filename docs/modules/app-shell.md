...
通知引导细节候选（2026-09-13，待负责人验收）：登录且设备后台提醒开启后，在已恢复前台的应用壳显示一次可跳过的非阻断提示；未授权时显示“开启消息通知，及时查看私聊和回复”，只有点“允许通知”才弹系统授权框。授权后仅提示开启“新消息提醒”的悬浮通知，不再解释系统提示音或常驻类别；点“去设置”才离开应用。拒绝后保留前往设置的选择，不自动重试，不阻断页面使用。游客、关闭功能、后台或不支持的平台不主动展示。退出/切号使旧引导及其迟到操作结果失效；系统授权框返回的生命周期刷新等待在途申请，不覆盖结果。账号设置长期提供手动申请与频道入口。
