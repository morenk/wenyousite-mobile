...
中央发布入口按当前频道缩短路径：首页直达“写主题”，动态直达“发动态”，消息与我的分支才显示“发布主题帖 / 发布动态”两个目标操作，不重复解释内容类型。
...
- [x] 四分支使用 IndexedStack 保留页面状态；底栏中央发布动作在首页/动态直达对应创作页，在消息/我的显示稳定类型选择，首页与动态顶栏可进入全站搜索。
