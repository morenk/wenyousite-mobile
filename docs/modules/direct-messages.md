...
capability 和前台生命周期由 app-shell 提供，入口由 notifications/users 提供，目标资料由 users 读取，图片上传由 media 完成，收藏选择与快速收藏由 stickers 提供，邮箱验证由 auth/settings 恢复。视觉只复用 Foundation v1.1.0 的 Token、面板、状态横幅、按钮和最小触控目标，不维护移动端平行审美规范。私聊图片不得复用 Markdown 正文解析器；表情发送复用本模块 `stickerAssetId` 的独占消息约束。
