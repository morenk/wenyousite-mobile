...
实现服务端 capability 控制的站内一对一私聊：会话、消息请求、历史与增量消息、首条消息、后续发送、接受/拒绝、归档、已读、撤回、收到消息举报、未读角标和 Android 默认开启且设备级可关闭的后台系统顶栏提醒形成完整移动闭环。当前不实现 FCM 后台送达、离线消息队列或本机私聊正文持久化；后台提醒只拉取会话元数据且不显示消息正文。输入器已通过 stickers 模块选择收藏表情，消息图片或表情也可加入本人收藏。

私聊选图仅显示本地预览，点击发送才上传。已有会话立即出现本地图像气泡，等待超过 300ms 时在图内遮罩转圈，不再在气泡外重复转圈；首条私聊仍在输入框预览图内等待。失败图点按可重试发送或删除，输入框不展示上传阶段行和百分比。
...
选择控件修缮（2026-09-19，负责人验收通过）：会话列表筛选复用统一选择菜单。会话更多操作使用统一面板与紧凑图标文字行；归档及恢复行为保持现有权限和请求边界。 验收清单见[选择控件修缮](../architecture/selection-controls-acceptance.md)。
...
会话、请求与归档列表中的最新消息安全摘要最多显示四行，超出后在第四行末尾省略；进入会话后聊天气泡继续显示完整消息，不受列表摘要上限影响。

发送会立即插入本地气泡并清空输入器；图片气泡直接消费本地字节预览，上传完成后才用同一 `clientRequestId` 创建服务端消息。HTTP 成功按幂等键原位替换，图片准备、上传或发信失败只标记对应气泡且可复用原输入与幂等键重试。完整文字气泡使用共享站内传送门阅读组件；长按复制保留普通正文，只把行内原子替换为可见 label，不复制站内目标或协议节点。会话与首条消息页面底部复用单体内联输入 dock，输入器只提供普通文字和站内传送门原子，站外链接、非法邀请和混合剪贴板保持普通文字。

点击消息区或输入框外会收起软键盘并保留未发送内容。收藏表情面板保留输入焦点、选区与键盘。消息长按在气泡旁打开操作气泡，复制、收藏、撤回、举报、重试和删除按权限出现。收到的请求图片默认隐藏，用户点按或接受请求后才加载；拒绝需二次确认。已接受会话和发出的待处理请求可归档，自己十分钟内的消息可二次确认撤回，撤回待处理首条消息会取消请求并退出会话。

五分钟内同一发送者的连续消息使用 8dp 纵向间距并保留连续气泡尾角；切换发送者或跨越五分钟时间组使用 16dp，时间标签到下一条气泡保持 8dp。最新消息与输入 dock 之间沿用既有底部留白。

新私聊页同时读取已有会话事实和目标公开资料。互相关注时直接在收件人下方展示输入器，不再堆叠说明面板；其他可发起用户只在输入器上方显示一条紧凑的“这会先作为消息请求”提示。资料关系只用于发送前文案，新会话的 `ACCEPTED/PENDING` 最终仍以创建接口回包为准。复制、收藏表情、接受/拒绝请求、归档和撤回的短反馈使用 2.5 秒 floating SnackBar，失败说明使用 4 秒；底部同时留出键盘、安全区和输入 dock 间距，不覆盖输入框，新反馈替换旧反馈。
...
归档/取消归档立即更新当前会话的归档状态，明确失败只恢复归档字段，避免覆盖消息、已读和发送状态；结果不明读取会话确认。开始提交已读时立即清本会话已读部分，并通过独立角标控制器扣减已接受会话的相应消息数量，待处理请求数保持不变；旧角标查询失效，在途期间暂缓计数轮询，结算后读取最新总数以纳入新消息。失败恢复本次扣减，回调只能结算一次。已于 2026-09-13 获负责人确认通过并授权合并，见 [乐观更新验收](../architecture/optimistic-interactions-acceptance.md)。
...
未读控制器只负责单次读取；应用壳在认证且前台时用唯一的 30 秒定时器统一触发通知、未读消息与待处理请求刷新。私聊页签可见时，每次未读总数成功刷新后同步当前会话列表，以服务端的单会话 `unreadCount` 更新列表角标；切回私聊页签、返回底栏“消息”及恢复前台也会重新读取已打开的会话列表。Android 后台提醒默认开启且可按设备关闭；离开前台后，后台协调器仅在 specialUse 常驻服务确认运行时每 30 秒检查，前一轮未完成时跳过节拍。每轮先读未读/请求总数，只有计数增长才按需读取 INBOX/REQUESTS 首屏并比较会话 `lastMessage.id + unreadCount + status` 指纹；详情读取或系统卡片展示失败时不推进未读与会话指纹，下一节拍继续重试。恢复前台、退出登录、任务被划掉或进程被系统回收即停止。
...
三条路由都要求 authenticated，会话权限完全以服务端 `canSend/canAccept/canDecline/isBlocked` 和状态投影为准。客户端不记录完整私聊正文、图片预签名 URL、Token 或消息请求载荷；正文、失败草稿和上传中的媒体只存在于页面进程内。后台系统卡片标题只显示服务端会话投影中的对方用户名，正文固定为“新私聊”或“新的私聊请求”，绝不消费 `lastMessage.content`；锁屏可见性为 private。陌生请求图片在明确查看前不发起网络加载；撤回响应仍含正文或媒体时拒绝展示。举报由 reports 模块提交，后端再次复核当前用户确为消息接收者；举报表单和日志不复制完整私信正文。契约 4.14 后所有注册用户使用统一身份，私信写入不再检查邮箱验证状态。`40305/40306`、`40411/40412` 与 `40906`～`40909` 使用稳定业务提示。

## 8. 本地存储、缓存及失效规则

会话列表、会话内容与未读数依赖 `ViewerScope`，切号或拉黑关系变化释放旧缓存及轮询；Token 刷新不重建。旧控制器的迟到请求不得恢复前一查看者的消息。

私聊正文、会话列表、未读数、后台会话指纹、失败幂等草稿和增量锚点不写 Drift、SharedPreferences 或文件；页面销毁、退出登录、切号或进程终止即释放。图片使用共享安全 HTTP(S) 缓存；发送前的输入器和发送后的会话任务只在内存短暂保留源字节，失败时供对应附件或气泡重试，移除附件、放弃失败气泡、成功或页面释放都会清除。回前台、进入中心、会话已读/请求处理/撤回和下拉刷新会重新读取服务端事实；应用终止后的失败首条消息不会后台自动重放。

## 9. 加载、空数据、错误、重试和冲突状态

拉黑与解除后清除会话及未读缓存。会话刷新、增量轮询和历史分页收到 403/404 时清除旧消息与会话投影，显示不可访问，不能继续展示历史消息。
...
capability 由 app 组合层从启动契约注入，前台生命周期由应用壳承接；入口由 notifications/users 提供，目标资料由 users 读取，图片上传只依赖 media application 端口和任务控制器，收藏选择与快速收藏由 stickers 提供，举报表单与提交由 reports 提供，auth 只提供统一登录会话。私聊 presentation 不导入 media data、reports data、Dio 或 app-shell 的具体 provider。视觉只复用 Foundation v6.9.0 的 Token、图片契约、语义图标、状态横幅、按钮和最小触控目标；会话是带分隔线的连续列表，未读通过 `destructive / onDestructive` 计数角标表达，高 16dp、10sp 粗体等宽数字，超过 99 显示 `99+`，不用逐条面板制造卡片层级。私聊图片不得复用 Markdown 正文解析器；表情发送复用本模块 `stickerAssetId` 的独占消息约束。

## 11. 测试场景与验收条件

- [ ] 后台常驻与系统顶部横幅候选：保留原消息频道及用户偏好，服务停止/切号后的旧结果不得展示。双消息、横幅关闭/勿扰/锁屏和持续 30 分钟的真机结果见[候选验收](../architecture/background-reminders-acceptance.md)。
...
- [x] 会话、请求与归档列表的 `contentPreview` 最多显示四行并省略超出内容，详情消息气泡保持完整展示。
- [x] 完整私信正文中的具名、裸地址和历史 URL 自标签站内引用渲染为可导航传送门；非法邀请与外链保持字面文本，传送门长按仍打开消息操作，复制只输出原子可见 label，会话列表继续只消费服务端脱敏预览。
- [x] 私聊粘贴完整合法主题或邀请链接时在选区插入可混排、整粒选择/复制/删除的传送门原子并提交规范 Markdown；输入器不带格式工具栏、字数提示或独立草稿层，混合剪贴板与非法链接不转换。
...
- [x] 未读总数刷新及切回私聊时重新读取当前会话列表，单会话角标与服务端最新 `unreadCount` 对齐；原设备反馈仍待负责人在候选 APK 上复验。
- [x] 中心、会话、新私聊的主路径、错误恢复与 320dp、360dp、400dp、600dp 布局通过 Widget 测试；会话/请求/归档使用紧凑筛选菜单，默认锚点仍显示请求未读数，并在在途时禁止重复切换。内联输入 dock 的正文独占首行，表情能力开启时仍在 280–320dp 软键盘 inset 及 1.3/2.0 倍字体下保持可见且不提前换行，并在 IME 最终 inset 可用时一帧到位，不跟随系统键盘动画逐帧升降；点击消息区会失焦收起键盘但保留当前输入，360dp 键盘态与 600dp 最新消息底部对齐的分组气泡由 golden 固定。长会话首次进入直接显示最后一条，连续气泡、消息旁长按操作、保留键盘的收藏表情锚点面板、离底新消息浮标和历史锚点保持可操作。
...
当前用 HTTP 固定 30 秒尽力轮询读取新消息；Android 仅在 Flutter 进程仍存活且继续获系统调度时把会话元数据变化显示为系统通知，使用静音 specialUse 常驻服务改善后台运行条件，但不做 WebSocket、SSE、FCM 后台唤醒或实时送达保证。Doze、厂商省电、断网或进程终止可延迟/停止检查，重新进入页面后读取权威事实。计数未增长但发生跨设备已读与新消息抵消时可能不触发系统卡片。后端未投影单条消息的送达、已读、输入中或在线状态，移动端只展示可证实的发送中、失败与服务端已确认，不绘制双勾或伪在线。输入器支持文本、单张图片与独占收藏表情；已有会话的三类消息都立即生成乐观气泡，图片仍因服务端契约要求先取得 `mediaId`，所以本地气泡只是当前进程内的发送任务，不代表服务端已经收到。新建首条私聊尚无会话时间线，图片等待仍显示在输入器。失败幂等消息和源字节不跨重启保存，因此不明确失败后若用户终止应用，应先重新查看会话事实而不是生成新请求盲发。
...
端口、控制器与状态：`lib/features/direct_messages/application/`；API 适配器：`lib/features/direct_messages/data/`；页面：`lib/features/direct_messages/presentation/`；后台会话指纹与卡片：`lib/features/app_shell/application/background_online_poller.dart`。参见[导航](../architecture/navigation.md)、[网络与会话](../architecture/networking.md)、[站内通知](notifications.md)、[用户与资料](users.md)、[媒体](media.md)、[社区举报](reports.md)、[语义图标](../architecture/icons.md)和[Foundation v6.9.0 Flutter profile](https://github.com/morenk/wenyousite-foundation/blob/v6.9.0/docs/platforms/mobile.md)。
