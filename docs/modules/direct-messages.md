...
私聊图片从相册选择后不打开裁剪页，静态图与 GIF 都直接以 `DIRECT_MESSAGE` 用途进入既有准备和上传流程；选图取消、上传失败或上传取消都不改动当前文字、选区或焦点。
...
- [x] 图片选择后不经裁剪直接上传，失败保留正文、选区与焦点；同一原图重试、完成后才可发送、上传中取消以及裁剪页不出现均有页面回归测试。
