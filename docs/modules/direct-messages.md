...
普通私聊图片使用服务端返回的宽高在网络请求前按真实比例占位，在气泡可用宽度与 280dp 高度内完整呈现，不裁切或拉伸；纯图片继续保留本人粉色、对方灰色的气泡外框。收藏表情同样完整呈现，长边不超过 Foundation 的 128dp。预览优先使用服务端显式提供的中图或缩略图，点按后进入共享全屏原图页，支持系统返回、双击/捏合缩放、平移和未放大时下滑关闭。
...
`DirectMessageMedia` 保留 API 已有的可空 `width/height`。两者有效时 presentation 计算 contain 尺寸并只向图片解码器传递实际限制轴，避免宽高同时固定造成变形；尺寸缺失时不猜测比例，先显示角色占位，图片解码后使用固有比例。
