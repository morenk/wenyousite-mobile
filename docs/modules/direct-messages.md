...
组件统一第一批（2026-09-19，候选／待负责人验收）：拒绝消息请求和撤回使用共享确认框，保留操作后果说明；接受／拒绝按钮统一处理中反馈，更早消息使用共享分页状态并保留逆序列表及原游标。 入口清单、自动检查及真机步骤见[组件统一验收](../architecture/component-consistency-acceptance.md)。
