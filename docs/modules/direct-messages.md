...
归档/取消归档立即更新当前会话的归档状态，明确失败只恢复归档字段，避免覆盖消息、已读和发送状态；结果不明读取会话确认。开始提交已读时立即清本会话已读部分，并通过独立角标控制器扣减已接受会话的相应消息数量，待处理请求数保持不变；旧角标查询失效，在途期间暂缓计数轮询，结算后读取最新总数以纳入新消息。失败恢复本次扣减，回调只能结算一次。候选待负责人真机验收，见 [乐观更新验收](../architecture/optimistic-interactions-acceptance.md)。
