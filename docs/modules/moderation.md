...
治理决定页不重复顶部标题，保留申诉期限与次数约束。申诉表单使用共享 Sheet 和可滚动内容，键盘出现后提交仍可达，权限、凭据与重复提交保护不变。
...
设计统一后三批候选／待负责人验收：治理决定与申诉通道到期使用 Foundation 精确时间，直接显示完整本地年月日及时分；凭据到期不再只显示 HH:mm。安全行为、凭据清除和提交规则保持。 交付及手测见[三批统一验收](../architecture/design-unification-completion.md)。
...
组件统一第一批（2026-09-19，负责人验收通过）：申诉入口及表单提交接入共享异步按钮，保持原提交锁、隐私凭据和退出保护。 入口清单、自动检查及真机步骤见[组件统一验收](../architecture/component-consistency-acceptance.md)。
...
2026-09-23 契约同步复核：Backend `92b030a81f8957386e324fed477bd1e46faf65ea`、OpenAPI `5.26.0-dev.20260922.3`，与公网 `/meta` 一致；新增管理内容整理接口不适用于移动端，本模块既有社区举报行为不变。
