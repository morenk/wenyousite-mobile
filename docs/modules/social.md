...
统一主题与动态收藏、两套私有收藏夹目录、关注、订阅、点赞和拉黑的读写状态。当前已交付主题点赞、主题/动态首次收藏选夹、两类独立收藏夹目录与内容管理、两类订阅、公开用户页关注与拉黑，以及公开/本人关注、粉丝和本人黑名单列表；自建收藏夹重命名／删除仍为待负责人真机验收的移动端候选，公开用户动态收藏列表仍是后续切片。收藏表情由独立 stickers 模块负责，本模块不包含推荐算法配置。
...
- [ ] 自建主题／动态收藏夹管理候选：默认夹不显示管理入口；自建夹可预选并改名、删除；校验、写入失败保留输入/原列表，成功删除严格采用响应目标 ID 并将内容留在默认夹。9 个定向测试文件共 87 项通过；全量 Flutter 测试运行中有一个无关的 `controlled_cover_animation_test.dart` 图像实例断言失败，单独复验通过。`check:apk` 被尚未发布的 Backend PR #20 与公网契约版本差异挡在生产校验阶段，未构建 APK；待后端发布后完成完整门禁和负责人真机验收。
- [ ] 切号不泄露关系和拉黑列表。

## 12. 已知限制和后续功能

当前未建立跨页面互动事件总线，主题与动态收藏、订阅变化不会主动改写其他仍存活页面。公开用户动态收藏虽已有服务端契约，但移动端尚未接入。后端订阅列表暂无按主题/类型筛选和分页参数，移动端目前必须读取本人完整订阅后过滤；数据量增长前需要补齐契约。自建收藏夹管理所需的 Backend PR #20 已合并并部署（`e214fd18637cb10d79576c5ab5a4cf42340fef71`，API `5.23.0-dev.20260913.1`）；移动端真实账号写入路径与真机交互仍待负责人验收。不做离线关系队列或推荐偏好；视觉动效后置。

## 13. 最近审查的契约版本和后端提交

2026-09-20 自建收藏夹管理候选契约复核：Backend `e214fd18637cb10d79576c5ab5a4cf42340fef71`，OpenAPI `5.23.0-dev.20260913.1`，与公网 `/meta` 一致；四个新增操作均已在本候选接入，负责人真机安装包仍待核对。自动检查、APK 与手测路径见[候选验收](../architecture/bookmark-folder-management-acceptance.md)。
