...
本人收藏页打开后直接展示主题默认收藏夹内容；“主题 / 动态”页签切换两套独立目录，右侧紧凑收藏夹筛选器原地选择默认夹或自建夹，并显示服务端数量。新建、移动或取消收藏成功后同步刷新当前列表和筛选器数量；刷新失败保留可读内容并提供带问题编号的局部重试。主题内容复用 thread_feed 标准信息流卡片，完整呈现默认子贴活跃时间、摘要、安全首图、主题标签、玩家数及其他互动统计；动态收藏复用 moments 双列瀑布卡片，每张卡以 48dp“更多”入口打开当前收藏夹、移动和取消收藏操作，不在窄卡内堆叠管理按钮。
