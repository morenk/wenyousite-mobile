...
统一主题与动态收藏、两套私有收藏夹目录、关注、订阅、点赞和拉黑的读写状态。当前已交付主题点赞、主题/动态首次收藏选夹、两类独立收藏夹目录与内容管理、两类订阅、公开用户页关注与拉黑，以及公开/本人关注、粉丝和本人黑名单列表；自建收藏夹重命名／删除候选已由负责人真机验收通过，公开用户动态收藏列表仍是后续切片。收藏表情由独立 stickers 模块负责，本模块不包含推荐算法配置。
...
个人中心收藏夹从独立个人工具区进入，与编辑资料分开；公开主页保留遵循隐私设置的公开收藏标签。本人关系列表的关注／回关与已关注／互相关注使用紧凑按钮表面，并保留至少 48dp 点击区域；各状态右侧始终保留更多菜单，状态变化不移动菜单位置。已关注状态按钮仍可打开同一关系菜单。关系、订阅列表使用共享等级徽章，分页结束默认不追加占位文案，失败与继续加载仍可操作。
...
本人关系管理紧凑列表（负责人于 2026-09-24 确认验收通过）：从 `/me/following`、`/me/followers` 或本人公开主页进入，返回和“关注／粉丝”数量页签合并同一顶栏，分别保留滚动位置。列表取消卡片与固定第二操作行，40dp 头像、body 中等字重昵称、caption 等级、昵称起细分隔线；普通行约 65dp，大字空间不足时身份和操作自然换行，姓名省略但保留完整语义。回关／关注复用 filled 主按钮，已关注／互相关注复用共享 tonal 等宽状态按钮；状态或更多打开同一底部菜单，命中区至少 48dp。关系菜单复用共享 `showWenyouSheet` 的宽度、安全区、键盘和高度约束，保留身份标题及按会话受控关闭。主／状态按钮的实际读屏语义包含用户名、动作及菜单用途，处理中仍保留目标。他人公开列表、黑名单与原深链保留原行为。
...
选择控件修缮（2026-09-19，负责人验收通过）：收藏页的收藏夹入口复用 app-shell 共享选择菜单，统一入口间距、长名称换行、右侧勾选及选中底色；选择不同收藏夹仍沿用原来的分页重置和计数更新。验收见[选择控件修缮](../architecture/selection-controls-acceptance.md)。

本人收藏页打开后直接展示主题默认收藏夹内容；“主题 / 动态”页签切换两套独立目录。收藏夹筛选器与内容列左对齐，旁边显示当前服务端数量；长名称单行省略，展开后选择默认夹或自建夹。新建、移动或取消收藏成功后同步刷新当前列表和筛选器数量；刷新失败保留可读内容并提供带问题编号的局部重试。主题内容复用 thread_feed 标准信息流卡片，完整呈现默认子贴活跃时间、摘要、安全首图、主题标签、玩家数及其他互动统计；动态收藏复用 moments 双列瀑布卡片。

收藏页布局候选（2026-09-11，待负责人验收）：主题卡片标题右侧以 48dp“更多”入口替代原卡片外移动按钮及取消图标，点按不触发详情导航；动态保留原卡片内入口。两类收藏共用底部管理面板，以只读辅助信息显示当前收藏夹，操作统一为“移动到其他收藏夹”“取消收藏”。关闭不写入，移动进入现有选夹流程；写入期间禁用重复管理并在当前项显示加载状态。主题列表筛选到内容的顶部间距收紧为 space8，不改变封面或四行摘要密度。复验记录见[收藏页布局候选验收](../architecture/bookmark-list-ux-acceptance.md)。

主题与动态详情都以服务端投影初始化互动。首次收藏先读取对应类型目录，默认收藏夹预选；用户也可选择自建收藏夹或在 Sheet 内新建，只有点击“确认”才携带 folderId 发出一次创建收藏写入，不再先写默认夹再移动。目录加载、创建和收藏写入失败都留在原 Sheet 重试；已收藏内容点击仍直接取消。两类目录、模型和计数完全独立，不读取主题目录中的旧动态兼容计数。服务端默认收藏夹保持第一项，新建名称发送前 trim 并校验 1–24 字。主题和动态内容页都以必填 folderId 从首屏读取并建立独立分页状态；移动或取消确认后立即从当前目录移除，主题目录同时按本次移动/取消增减数量；成功后重新读取当前内容页首屏和对应目录计数，禁止从收藏记录 ID 构造游标。订阅控制器独立收敛本人订阅和帖内成员候选：本人订阅成功后即可操作官方更新，候选读取失败只影响玩家入口并可单独重试；官方/玩家订阅都按记录 ID 取消。点赞采用服务端计数；点赞、收藏、订阅、关注与拉黑在写入结果不明时读取对应详情、订阅列表或用户关系投影，只有投影证明目标已达成才同步为完成。点赞/收藏统一复用 Foundation v7.0.0 语义色、选中表面、实心资产和精确在途动作反馈。

菜单按私聊（capability 开启）、取消关注（已关注）、移除粉丝（对方关注）、拉黑、举报排序；互关用户两页签都提供取消和移除。先关闭菜单再进入原私聊、共享举报或确认。social 通过 reports.dart 公共 facade 复用举报流程，正式声明 social→reports 依赖，不新增内部导入债务。拉黑复用已有确认语义，不删除双向关注，只刷新服务端可见性；每个 target 的关注、移除、拉黑共享行锁。拉黑及解除请求禁止网络重放，未知拉黑必须读取 projection.isBlocked 核实，列表消失不足以证明拉黑；其他入口解除后更新菜单投影。
...
本人两页签共用 autoDispose 控制器和行锁，写成功同步移行、互关及数量，保留其他行顺序；切换会话重建控制器，迟到结果不更新新账号。组合根同时失效本人资料、本人公开资料、目标公开资料及旧公开关系列表，不通过全局可见性刷新销毁正在操作的本人双列表。从用户详情返回时读取两页最新关系，滚动位置由页签保留。确认框与菜单由入口传入固定会话，首帧前切号即拒绝旧上下文；切换账号或目标失效时仅关闭自己所属路由，不误关其他覆盖页面。可见性版本仅触发受控刷新，不重建本人控制器或清空其他目标在途锁；被写入打断的刷新恢复读取，过期读取不得回退最新投影。
...
拉黑/解除确认成功或结果不明确时触发应用级可见性缓存失效；从黑名单解除也触发同一入口。内容、搜索、资料、关系、收藏、通知及私聊重新读取当前查看者投影，编辑器草稿保留。
...
主题收藏的独立卡片复用 Foundation v7.1.2 内容卡片 10dp 圆角和 8dp 卡间距；收藏管理面板使用共享卡片 10dp 圆角，页面边距保持原值。

所有主题封面统一复用[只读主题信息流](thread-feed.md)的首帧、停稳中心单张播放、省流量与生命周期规则；本模块不另建动画调度器，点击继续进入详情。


本模块页面排版统一遵循[移动端视觉基线](../architecture/visual-baseline.md)中的 Foundation v7.0.0 语义文字角色，不自定义字号或直接依赖 Material 字体槽位。
...
本人关系管理回归覆盖单向与互关、移除保留反向、两页计数与资料失效、重复点击、并行不同用户操作、旧刷新、未完成初始加载、退出会话、不明结果只读核对、本人公开入口与他人边界、确认取消/失败、页签位置恢复，以及 320dp/双倍文字/长姓名下等宽主／tonal 状态、自然换行和 48dp。新增回归覆盖双方菜单顺序、私聊 capability、同步重复关闭与覆盖路由保留、拉黑未知核实、可见性打断恢复、其他入口解除拉黑和旧投影晚到。证据和人工步骤见[关系管理候选验收](../architecture/relations-management-acceptance.md)。所有写入自动化在本地模拟仓储或已验证的隔离环境运行，公网仅只读。
...
- [x] 自建主题／动态收藏夹管理：默认夹不显示管理入口；自建夹可预选并改名、删除；校验、写入失败保留输入/原列表，成功删除严格采用响应目标 ID 并将内容留在默认夹。合入最新 `dev` 后完整门禁与 Debug APK 通过，负责人于 2026-09-20 确认最终候选真机验收通过；正式发布仍另行执行。
- [ ] 切号不泄露关系和拉黑列表。

## 12. 已知限制和后续功能

当前未建立跨页面互动事件总线，主题与动态收藏、订阅变化不会主动改写其他仍存活页面。公开用户动态收藏虽已有服务端契约，但移动端尚未接入。后端订阅列表暂无按主题/类型筛选和分页参数，移动端目前必须读取本人完整订阅后过滤；数据量增长前需要补齐契约。自建收藏夹管理所需的 Backend PR #20 已合并并部署（契约 `5.23.0-dev.20260913.1`），Mobile 最终 Debug 候选由负责人真机验收通过；正式 Release 构建和发布尚未执行。不做离线关系队列或推荐偏好；视觉动效后置。
...
2026-09-23 契约同步复核：Backend `92b030a81f8957386e324fed477bd1e46faf65ea`、OpenAPI `5.26.0-dev.20260922.3`，与公网 `/meta` 一致；本人关系列表新增投影和移除粉丝端点尚未接入，既有社交行为不变。
...
2026-09-13 自建收藏夹管理候选同步：Backend PR #20 `df4682548e3fc0291fc2cd19c7111b5f0fa53746`，OpenAPI `5.23.0-dev.20260913.1`，新增主题与动态收藏夹重命名／删除。PR 尚未合并，公网仍为 `5.22.0-dev.20260912.2`；本轮按 PR 固定契约生成并实现客户端，依赖后端→Web→移动端顺序合入和发布。Foundation 已核对正式 Tag `v7.0.0`，与当前锁定版本一致。9 个定向测试文件共 87 项通过；完整门禁在生产契约校验时因公网仍为旧版本而停止，Debug APK 与负责人真机验收待后端发布后完成。
...
契约 `5.18.0-dev.20260905.1`；Markdown v5；后端 `3338028459561565c788d5236fb64db84a2ae538`；Foundation `v6.9.0`（`5888132`）。主题帖目录不再消费已废弃的动态兼容计数，动态目录改用独立端点。
