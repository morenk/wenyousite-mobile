...
统一主题与动态收藏、两套私有收藏夹目录、关注、订阅、点赞和拉黑的读写状态。当前已交付主题点赞、主题/动态首次收藏选夹、两类独立收藏夹目录与内容管理、两类订阅、公开用户页关注与拉黑，以及公开/本人关注、粉丝和本人黑名单列表；自建收藏夹重命名／删除属于依赖 Backend PR #20 的移动端候选，公开用户动态收藏列表仍是后续切片。收藏表情由独立 stickers 模块负责，本模块不包含推荐算法配置。
...
当前未建立跨页面互动事件总线，主题与动态收藏、订阅变化不会主动改写其他仍存活页面。公开用户动态收藏虽已有服务端契约，但移动端尚未接入。后端订阅列表暂无按主题/类型筛选和分页参数，移动端目前必须读取本人完整订阅后过滤；数据量增长前需要补齐契约。自建收藏夹管理依赖 Backend PR #20 的新接口（本地契约 `5.23.0-dev.20260913.1`）；PR 合并并发布前，当前公网服务仍为 `5.22.0-dev.20260912.2`，线上操作不可用。上线顺序须先完成后端、Web、移动端接入；本候选没有后端真实环境联调，也待负责人真机验收。不做离线关系队列或推荐偏好；视觉动效后置。
