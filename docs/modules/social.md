...
选择控件修缮候选（2026-09-19，待负责人验收）：收藏页的收藏夹入口复用 app-shell 共享选择菜单，统一入口间距、长名称换行、右侧勾选及选中底色；选择不同收藏夹仍沿用原来的分页重置和计数更新。验收见[选择控件修缮](../architecture/selection-controls-acceptance.md)。
