...
组件统一第一批（2026-09-19，候选／待负责人验收）：主题收藏列表统一分页错误、加载和结束提示，空列表及刷新时隐藏结束提示；关系解除和收藏管理使用共享异步按钮。分页组件继续服从原 isBusy 条件，不改乐观写入或待确认结果处理。 入口清单、自动检查及真机步骤见[组件统一验收](../architecture/component-consistency-acceptance.md)。
