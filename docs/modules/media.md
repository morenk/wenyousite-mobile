...
提供正文与头像图片选择、预签名上传、确认、完成态轮询和安全查看。当前已完成创建主题编辑器图片及本人头像设置/移除闭环；V1 不做视频、后台上传或离线上传队列。
...
`/compose/thread` 工具栏的图片按钮打开系统相册，随后要求填写不依赖文件名的替代文字。上传完成后在当前光标附近插入块级图片节点，正文图片轻触进入全屏页。“我的” `/me` 本人总览提供选择/更换头像；已有头像额外提供二次确认移除。
...
只有 `COMPLETED` 资源能成为 `wenyou_image` 节点或头像候选；编辑器负责 alt、插入位置与最终 Markdown 快照，users 负责头像端点和本人资料事实。正文图片遵循 Foundation 的 contain、不裁切、状态占位和全屏交互契约；头像是允许 cover 裁切的独立角色，以圆形视图和统一占位展示。
