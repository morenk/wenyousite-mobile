...
- [x] Android 相册入口启用系统 Photo Picker；头像和主页背景选图失败可见且可重新选择，用户主动取消不误报。

## 12. 已知限制和后续功能

服务端用户 DTO 未提供 thumbnail/medium 字段，因此移动端只使用明确原地址。原图、预览、取景框与失败上传输入只在当前页面/autoDispose 生命周期内短暂保留，不写入本地快照或业务状态，也不提供后台队列；页面释放、主动取消或进程终止后需重新选择图片。相册文件访问由 `image_picker` 与系统 Photo Picker 管理。
