...
组件统一第一批（2026-09-19，候选／待负责人验收）：恢复上次图片选择与选择失败重试使用共享确认框，保留嵌套导航和禁止遮罩关闭；裁剪确认接入共享异步按钮，不改变裁剪、上传或取消过程。 入口清单、自动检查及真机步骤见[组件统一验收](../architecture/component-consistency-acceptance.md)。
