...
- [x] 共享上传控制器覆盖进度、取消、失败、重试、迟到结果丢弃与页面释放；主题、帖子和收藏表情覆盖选择、裁剪、上传及同裁剪文件重试，动态发布/编辑、动态评论和私聊覆盖不经裁剪的选择、上传、完成结果消费、同原图重试和取消；动态发布额外覆盖多图顺序处理。
...
- [x] 主题、帖子和收藏表情覆盖“选图 → 裁剪 → 上传”；动态发布/编辑、动态评论和私聊覆盖“选图 → 直接上传”且不会出现裁剪页，动态发布额外覆盖多选、原顺序串行上传和剩余数量限制。
