...
选图、预览解码或裁剪生成失败时，当前弹窗显示明确错误，并提供重新选择或原地重试。主页背景选择或上传失败后，页内错误横幅主动滚入视野，并根据已保留的裁剪结果和 `mediaId` 显示“重新选择 / 重试上传 / 重试设置”，不会静默失败。
