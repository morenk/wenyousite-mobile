...
图片上传体验候选（待负责人验收）：准备并发 1，直传/确认并发 2，确认后立即释放名额；每次状态查询独立使用并发 2 的查询通道，退避等待不占通道。主题、楼层/回复、动态和评论使用本地预览与逐图状态，300ms 后才显示动效，百分比只代表传输；完成无成功提示，失败显示“未完成”，超时显示“图片准备较久”。服务端完成语义、安全检查和图片质量不变。见[候选验收与耗时分析](../architecture/image-upload-experience/acceptance.md)。
