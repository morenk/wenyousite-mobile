...
完整展示候选：StickerAsset 携带完整 display，插入正文／编辑器、动态回复和私聊后使用完整 WebP；收藏导入参数与 markdown 仍为原资产身份。选择／管理面板保持既有静态 thumbnail 策略，不因格式选择开启动画。见[全场景候选记录](../architecture/animation-webp-all-surfaces.md)，待负责人验收。
