...
实现服务端 capability 控制的用户私有表情收藏：读取收藏与最近使用、从相册/私聊/帖子导入、查询异步处理、完整排序、移除，以及在私信和 Markdown 编辑器中使用。当前不做本机表情包文件夹、离线收藏、第三方表情市场或后台通知。
...
“我的”在 capability 开启时提供受保护路径 `/me/stickers`。私信输入器、主题/帖子编辑器打开同一收藏选择面板；帖子正文图片和已明确展示的私聊图片提供快速收藏。陌生消息请求图片在用户点按查看或接受请求前不显示收藏入口。
...
media 负责相册图片安全上传；direct_messages 负责 `stickerAssetId` 独占载荷和消息展示；editor 只插入 Codec 已支持的 sticker embed；posts/threads 传入真实 postId 与 Markdown 图片 URL。视觉只使用 Foundation v1.1.0 Token、面板、状态和最小触控目标，不维护平行审美规范。
...
- [x] capability 关闭、相册导入和 360/400/600dp 管理页布局有 Widget 测试。
