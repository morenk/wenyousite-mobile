...
从相册添加收藏表情时，裁剪确认后在收藏格内显示临时本地图，上传及加入收藏的等待都使用图内遮罩；临时格不参与拖动，完成后由正式收藏替换。重进页面只有服务端待处理记录时显示无本地图的占位格；失败图点按处理，重复收藏才给简短结果提示，普通新增不弹成功文案。
...
收藏表情加载失败操作统一为“重试”；个人中心入口位于独立工具区并继续受能力开关控制。
...
正文与动态全屏图集的收藏动作绑定当前显示图片的真实来源；正文使用 sourceId 与原 URL，动态/评论使用 sourceId 与 mediaId。图集未完成身份确认时不显示收藏入口，账号切换后丢弃迟到反馈。见[图片图集](../architecture/image-gallery-acceptance.md)。
