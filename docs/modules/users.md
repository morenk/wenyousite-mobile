...
本人中心使用主导航 `/me`：资料头保留 2:1 背景、头像、昵称、等级进度与数值、个性签名，以及关注、粉丝和私有温油余额。统计数字下方、昵称右侧提供小号描边“编辑资料”按钮；查看自己的公开页时复用同一按钮，进入既有 `/me/edit`。签名在昵称／等级下方独立左对齐，保留与头像的间隔。独立个人工具区承载收藏夹、表情包和油卡，油卡继续进入现有钱包页且不重复展示余额；表情包按能力开关显示，账号设置保留顶栏入口。内容区默认“主题”，同级标签为“主题 / 动态 / 参与 / 回复”，已访问分区用 IndexedStack 保留状态，不另设概览或收藏副本。`/me/bookmarks` 进入“主题 / 动态”统一收藏页及收藏夹筛选。`/me/edit` 仍以连续表单编辑头像、背景、用户名、简介和公开内容，底部唯一保存按钮提交表单；媒体与用户名按原流程独立保存。`/me/settings` 汇总偏好、资料、安全、关系和账号操作；“账号”分组首项“编辑资料”继续进入既有 `/me/edit`，与主页按钮复用相同编辑表单与保存流程。草稿仍留在对应编辑器内，不新增未实现的个人中心草稿入口。

主页结构候选／待负责人验收：本人及他人共用全宽资料表面，2:1 移动端封面与 72dp 头像衔接；无封面时不留占位。姓名与等级靠近，短经验条与明确数值在签名下方同一行显示，长姓名自然换行；大字号时统计与编辑按钮下移，不压缩可读文字。空简介、加入日期、邮箱和创作统计不占空间。公开内容标签固定在顶栏下，内层列表预留实际标签高度；主题默认加载，动态作为同级标签接入真实列表，参与、回复、收藏仍按公开权限出现。具体候选范围见[移动端体验重构验收](../architecture/mobile-ux-refactor-acceptance.md)。
...
资料头使用全宽单层表面，2:1 封面与 72dp 头像保持；普通工具与内容卡继续复用 Foundation v7.2.0 圆角和间距。头像与封面重叠的部分不再占据资料区高度，昵称靠近头像下方；统计保留最低触控区域并收紧上下留白。编辑按钮复用共享紧凑描边按钮，普通字号表面高 32dp、触控区域至少 48dp；签名单独左对齐并保留与昵称的间隔。经验条最长 96dp，与右侧经验数值居中对齐成一行；大字号可缩短经验条并允许文字自然换行，不裁切数值。大字号允许统计标签换行。个人工具区使用统一页边距；账号设置仍消费共享设置容器。
...
- [ ] 真机检查长内容滚动、页签来回、回顶展开资料头、封面／等级进度保留、签名间距与工具分组。
...
- [x] “我的”关注、粉丝与温油保持同级等宽统计；统计下方提供小号编辑资料按钮，账号设置的“账号”列表首项保留相同入口，两者复用原编辑页并可返回；收藏夹、表情包和油卡属于独立工具区，账号设置保留在顶栏。收藏进入统一页并直接展示默认夹，以类型页签和收藏夹筛选器原地切换；温油与油卡详情共享当前会话钱包状态。动态列表回到顶部后继续展开资料头，只有资料头和当前列表均位于顶部的新手势才同步资料、余额与当前页签，失败保留旧内容。
