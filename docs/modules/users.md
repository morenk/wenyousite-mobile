...
不做资料离线缓存、复杂勋章系统或后台用户管理。头像当前不提供手动裁剪；主页背景使用自动居中裁剪，不提供交互式选框。密码、邮箱、终端与注销已交由 settings/auth 的独立页面管理。“我的”本次不展示本地或云端草稿；勋章和跨页面关系同步后续接入，公开路由不会绕过关闭的隐私字段。
