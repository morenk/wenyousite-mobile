...
逐步实现用户主页、个人资料、隐私字段、创建/参与主题、最近回复、收藏、关注与粉丝列表。当前已提供公开资料与内容分区、公开及本人关注/粉丝列表、登录用户对他人的关注/拉黑操作，以及“我的”本人资料与公开范围编辑；账号安全写操作归 settings/auth。
...
公开用户主页使用稳定路径 `/users/:userId`，可从搜索结果和 Markdown 用户站内链接进入。关注和粉丝统计进入 `/users/:userId/following`、`/users/:userId/followers`；本人资料使用主导航 `/me`，并提供 `/me/following`、`/me/followers` 与 `/me/blocks`。创建、参与和收藏主题卡片进入 `/threads/:threadId`；最近回复进入 `/threads/:threadId?post=:postId` 并复用目标上下文定位。
...
不做资料离线缓存、复杂勋章系统或后台用户管理。“我的”尚不上传头像，也不管理密码、邮箱、终端或注销。用户动态、勋章、跨页面关系同步和本人私密内容筛选后续接入；公开路由不会绕过关闭的隐私字段。
