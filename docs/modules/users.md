...
公开用户主页使用稳定路径 `/users/:userId`，可从搜索结果和 Markdown 用户站内链接进入。统计区的“动态”进入 `/users/:userId/moments`；关注和粉丝统计进入指定用户路径；非本人页顶栏提供加油与举报入口，登录身份确认目标非本人且服务端 capability 开启时，“发私聊”进入 `/messages/new/:userId`。本人中心使用主导航 `/me`：资料卡直接展示移动端 2:1 背景、半嵌头像、简介和等级进度，并把“关注 / 粉丝 / 温油”作为同级等宽统计入口；“收藏”与“编辑资料”并列放在资料操作区。`/me/bookmarks` 纵向展示全部主题帖、动态收藏和完整收藏夹目录，主题帖内容使用 `/me/bookmarks/threads` 或 `/me/bookmarks/folders/:folderId` 独立打开。公开主页预览、表情包和账号设置位于顶栏次级入口，“温油”进入 `/me/wallet`。主页内容区不显示“我的内容”标题，居中展示“概览 / 动态 / 帖子”共享线性内容页签；帖子页内“创建的 / 参与的”二级页签也复用同一控件。两层都只响应明确点按，不安装横滑切页；已访问分区用 IndexedStack 保留页面状态。概览按 Web 端信息层级先展示“创作概览”的发布动态、创建主题、参与主题、累计回复四项计数，再展示最近回复。`/me/edit` 单独编辑双画幅主页背景、头像、用户名、简介和公开范围，`/me/settings` 汇总黑名单、登录终端、密码、邮箱、注销与退出。关系列表继续使用 `/me/following`、`/me/followers` 与 `/me/blocks`；用户动态路径继续作为稳定独立入口。创建、参与和收藏主题直接复用 threads 的公共主题信息流卡片，并进入 `/threads/:threadId`；动态与动态收藏复用 moments 信息流组件；概览中的最近回复进入带 `post` 查询的主题目标。本次不在“我的”内容区加入草稿。
...
- [x] “我的”居中展示“概览 / 动态 / 帖子”共享点按页签，帖子二级“创建的 / 参与的”同样复用该控件；不支持横滑误切，IndexedStack 保留已访问内容。默认概览并行加载创作概览与最近回复，公开开关不会隐藏本人内容，公开主页预览仍使用只读模式。
- [x] “我的”关注、粉丝与温油保持同级等宽统计；收藏移至编辑资料旁的独立操作，收藏总览纵向列出内容类型和全部收藏夹，再进入独立主题或动态列表；温油与钱包详情共享当前会话钱包状态。动态列表回到顶部后继续展开资料头，不触发刷新；只有资料头和当前列表均位于顶部的新手势才同步资料、余额与当前页签，失败保留旧内容。
