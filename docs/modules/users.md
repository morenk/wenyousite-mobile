...
组件统一第一批（2026-09-19，候选／待负责人验收）：用户名保存、退出当前账号接入共享异步按钮；公开创建／参与／收藏主题接入共享分页，嵌入内容无更多时不显示结束提示。用户名校验、等级与时间格式留待后续批次。 入口清单、自动检查及真机步骤见[组件统一验收](../architecture/component-consistency-acceptance.md)。
