...
通知设置文案精简候选：账号设置只保留“后台常驻提醒，可能增加耗电；划掉应用后停止”和“管理消息弹窗”等必要提示，移除默认提示音及轮询周期说明；不改变开关、申请权限或频道跳转行为。

账号设置通知引导候选（2026-09-13，待负责人验收）：通知未获允许时展示手动“申请权限”入口，拒绝后仍可重试或进入系统设置；已有权限不重复申请。系统设置入口使用“管理消息弹窗”简短说明。设备级首次引导由 app-shell 承载，不随账号变化重复，详见[通知引导验收](../architecture/notification-guidance-acceptance.md)。
