...
- [x] 一张安全图片保持原比例完整显示，可分别移动和缩小严格 3:1 与 2:1 取景框，确认后生成 1920 × 640 与 1600 × 800 双画幅并一次设置；取消、选图失败、部分上传失败、设置重试、移除确认和缓存失效正确，真实失败在小屏下也会主动显示并保留可用重试状态。
...
- [x] “我的”关注、粉丝与温油保持同级等宽统计；收藏移至编辑资料旁的独立操作，收藏页复用主题和动态列表；温油与钱包详情共享当前会话钱包状态，下拉刷新同步资料、余额、创作汇总和当前内容。
