...
本人中心使用主导航 `/me`：资料头保留 2:1 背景、头像、昵称、等级进度与数值、个性签名，以及关注、粉丝和私有温油余额。主页不再展示编辑资料按钮；签名在昵称／等级下方独立左对齐，保留与头像的间隔。独立个人工具区承载收藏夹、表情包和油卡，油卡继续进入现有钱包页且不重复展示余额；表情包按能力开关显示，账号设置保留顶栏入口。内容区默认“主题”，同级标签为“主题 / 动态 / 参与 / 回复”，已访问分区用 IndexedStack 保留状态，不另设概览或收藏副本。`/me/bookmarks` 进入“主题 / 动态”统一收藏页及收藏夹筛选。`/me/edit` 仍以连续表单编辑头像、背景、用户名、简介和公开内容，底部唯一保存按钮提交表单；媒体与用户名按原流程独立保存。`/me/settings` 汇总偏好、资料、安全、关系和账号操作；“账号”分组首项“编辑资料”进入既有 `/me/edit`，复用相同编辑表单与保存流程。草稿仍留在对应编辑器内，不新增未实现的个人中心草稿入口。

主页结构候选／待负责人验收：本人及他人共用全宽资料表面，2:1 移动端封面与 72dp 头像衔接；无封面时不留占位。姓名与等级靠近，经验进度显示明确数值并单独成行，长姓名自然换行；大字号时统计下移，不压缩可读文字。空简介、加入日期、邮箱和创作统计不占空间。公开内容标签固定在顶栏下，内层列表预留实际标签高度；主题默认加载，动态作为同级标签接入真实列表，参与、回复、收藏仍按公开权限出现。具体候选范围见[移动端体验重构验收](../architecture/mobile-ux-refactor-acceptance.md)。
...
资料头使用全宽单层表面，2:1 封面与 72dp 头像保持；普通工具与内容卡继续复用 Foundation v7.2.0 圆角和间距。头像与封面重叠的部分不再占据资料区高度，昵称靠近头像下方；统计保留最低触控区域并收紧上下留白。普通字号将经验数值右对齐在昵称／等级同一行，长昵称或大字号空间不足时自动另起一行；签名单独左对齐并保留与昵称的间隔，完整进度条紧随签名下方，不再为经验数值额外占据底部一行。大字号允许统计标签换行。个人工具区使用统一页边距；账号设置仍消费共享设置容器。
...
- [x] “我的”关注、粉丝与温油保持同级等宽统计；主页不设编辑资料按钮，编辑入口位于账号设置的“账号”列表首项；收藏夹、表情包和油卡属于独立工具区，账号设置保留在顶栏。收藏进入统一页并直接展示默认夹，以类型页签和收藏夹筛选器原地切换；温油与油卡详情共享当前会话钱包状态。动态列表回到顶部后继续展开资料头，只有资料头和当前列表均位于顶部的新手势才同步资料、余额与当前页签，失败保留旧内容。
