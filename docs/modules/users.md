...
主页背景选图、解码或上传失败都不会静默返回：裁剪阶段在弹窗中显示错误和原地重试，上传/设置阶段在页内显示带问题编号的失败横幅并自动滚入视野。页面依已保留状态给出重新选择、重试上传或重试设置，不重复已完成阶段。
...
- [x] 一张安全图片可分别拖动、缩放并确认严格 3:1 与 2:1 取景，生成 1920 × 640 与 1600 × 800 双画幅后一次设置；取消、部分上传失败、设置重试、移除确认和缓存失效正确，上传失败在小屏下也会主动显示并保留同图重试。
