...
- [x] “我的”关注、粉丝与温油保持同级等宽统计；编辑资料、收藏和 capability 控制的表情包使用资料卡底部透明等宽图标操作，账号设置保留在顶栏，不再提供公开主页预览。收藏总览只列出主题帖与动态两类入口，各自进入默认/自建收藏夹目录和指定夹列表，不展示跨夹“全部”；温油与钱包详情共享当前会话钱包状态。动态列表回到顶部后继续展开资料头，不触发刷新；只有资料头和当前列表均位于顶部的新手势才同步资料、余额与当前页签，失败保留旧内容。
