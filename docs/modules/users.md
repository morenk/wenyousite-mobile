...
主页结构候选／待负责人验收：本人及他人共用全宽资料表面，2:1 移动端封面与 72dp 头像衔接；无封面时不留占位。姓名与等级靠近，经验条与明确数值在签名下方共同占满一行，数值靠右、进度条填满剩余宽度，长姓名自然换行；大字号时统计下移，不压缩可读文字。空简介、加入日期、邮箱和创作统计不占空间。公开内容标签固定在顶栏下，内层列表预留实际标签高度；主题默认加载，动态作为同级标签接入真实列表，参与、回复、收藏仍按公开权限出现。具体候选范围见[移动端体验重构验收](../architecture/mobile-ux-refactor-acceptance.md)。
