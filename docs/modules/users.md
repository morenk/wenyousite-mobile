...
本人资料卡保留脱敏邮箱，不显示注册日期或“加入温油站”文案；公开资料卡不显示注册日期元信息。业务流水与内容的发生时间继续按各自模块规则展示。
...
公开与本人资料卡、创作概览及内容入口复用 Foundation v7.1.2 内容卡片 10dp 圆角；独立资料卡、内容卡与回复卡之间使用 8dp 卡间距，贴边背景图随资料卡外框裁切。内容页签、设置与操作面板沿用共享卡片的 10dp 圆角；账号设置及外观页的独立面板间距为 8dp，页面边距、导航行前留白与连续列表分隔线保持原值。
