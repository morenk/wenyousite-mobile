...
账号设置通知引导候选（2026-09-13，待负责人验收）：通知未获允许时展示手动“申请权限”入口，拒绝后仍可重试或进入系统设置；已有权限不重复申请。系统设置说明区分“新消息提醒”的悬浮通知/系统默认提示音与静音常驻通知。设备级首次引导由 app-shell 承载，不随账号变化重复，详见[通知引导验收](../architecture/notification-guidance-acceptance.md)。
