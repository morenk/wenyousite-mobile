...
本人中心使用主导航 `/me`：资料头保留 2:1 背景、头像、昵称、等级进度与数值、个性签名，以及关注、粉丝和私有温油余额。编辑资料位于头像右侧的数据区下方，签名在昵称／等级下方独立左对齐，不再紧贴头像。独立个人工具区承载收藏夹、表情包和油卡，油卡继续进入现有钱包页且不重复展示余额；表情包按能力开关显示，账号设置保留顶栏入口。内容区默认“主题”，同级标签为“主题 / 动态 / 参与 / 回复”，已访问分区用 IndexedStack 保留状态，不另设概览或收藏副本。`/me/bookmarks` 进入“主题 / 动态”统一收藏页及收藏夹筛选。`/me/edit` 仍以连续表单编辑头像、背景、用户名、简介和公开内容，底部唯一保存按钮提交表单；媒体与用户名按原流程独立保存。`/me/settings` 汇总偏好、安全、关系和账号操作。草稿仍留在对应编辑器内，不新增未实现的个人中心草稿入口。
...
- [x] “我的”关注、粉丝与温油保持同级等宽统计；编辑资料位于头像右侧统计下方，收藏夹、表情包和油卡属于独立工具区，账号设置保留在顶栏。收藏进入统一页并直接展示默认夹，以类型页签和收藏夹筛选器原地切换；温油与油卡详情共享当前会话钱包状态。动态列表回到顶部后继续展开资料头，只有资料头和当前列表均位于顶部的新手势才同步资料、余额与当前页签，失败保留旧内容。
