...
本人中心使用主导航 `/me`：资料卡展示移动端背景、头像、简介和等级进度，并把“关注 / 粉丝 / 温油”作为同级统计；编辑资料、收藏和表情包位于资料操作区，账号设置保留顶栏入口。`/me/bookmarks` 直接进入统一收藏页，以“主题 / 动态”页签和收藏夹筛选器原地切换内容；指定夹深链继续保留。“我的”内容区不显示重复标题，使用“概览 / 动态 / 创建 / 参与”四个同级点按页签，不再在“帖子”下嵌套第二层页签；已访问分区用 IndexedStack 保留状态。概览的四项创作统计在两列中分别居中；点击动态、创建或参与统计切换对应同级页签，点击累计回复定位到概览内的最近回复。`/me/edit` 使用单列连续表单：页首组合预览主页背景与头像，随后依次编辑用户名、简介和主页公开内容，底部保留唯一主保存按钮；`/me/settings` 汇总黑名单、终端、密码、邮箱、注销与退出。本次不在“我的”内容区加入草稿。

2026-09-26 个人区视觉合并候选：本人及他人共享资料头保留封面、头像、等级、统计和操作，采用 Foundation 10dp 内容卡圆角，但不叠加描边和多余分隔；空简介不显示提示，本人资料头不展示脱敏邮箱，双方均不显示加入日期。“概览”页签下的四项统计不再重复“创作概览”标题。资料编辑以白色分组面板承载基本资料与主页公开内容，仍由原保存按钮提交；浅色页底采用 Foundation 柔和背景，黑夜保持既有背景。页面滚动、入口、权限与失败恢复不变；业务流水与内容时间继续按各自模块规则展示。
...
公开页按用户 ID 读取资料，优先展示移动端 2:1 背景并安全回退；无背景时完全移除封面舞台。本人和他人主页共享全宽身份头、72dp 头像、等级与活动汇总。创作概览中的可见统计可直接进入用户动态或选中并定位到创建、参与、回复栏目；值为“未公开”时保持只读，不暴露对应入口。公开内容按隐私字段惰性请求；本人内容忽略公开展示开关，默认加载概览，动态、创建主题和参与主题在首次进入同级页签时读取，并保留已加载状态、游标和滚动位置。关系写入成功同步按钮、标记和粉丝数，私聊入口只传稳定用户 ID。本人主页只保留页面级下拉刷新，资料头与当前列表都位于顶部时才重读本人资料、钱包和当前页签。用户名从整行入口打开独立面板并立即保存；头像与主页背景确认取景后立即上传和设置，移除需二次确认；简介和主页公开内容只提交变化字段，由固定底部按钮统一保存。存在未保存表单时离页需明确放弃，媒体或用户名回写不会覆盖已有草稿。注销入口转交 settings/auth 完成不可逆确认和会话清理。
...
公开资料首次读取使用身份与正文分区结构 Skeleton；公开内容与关系列表首屏使用连续列表 Skeleton，资料、活动汇总和内容保持页面级与分区级空错恢复。关系列表提供加载、空、失败、请求 ID、重试和刷新。本人资料编辑加载中使用结构 Skeleton，失败时只显示明确重试，不提前露出外观或空表单；账号设置是独立入口，不因本人资料读取失败而被遮断。本人主页刷新失败不替换已显示的资料、余额或内容。资料、头像与背景写入失败保留原事实、当前草稿和请求 ID；媒体选择取消静默返回，上传可取消并按缺失步骤重试。用户名占用、7 天冷却和 429 以服务端为准；当前已有简介不能清空时明确拦截。

## 10. 跨模块约束

公开与本人资料卡、创作概览及内容入口复用 Foundation v7.1.2 内容卡片 10dp 圆角；独立资料卡、内容卡与回复卡之间使用 8dp 卡间距，贴边背景图随资料卡圆角裁切。资料头采用无描边单层表面；内容页签、设置与操作面板沿用共享卡片的 10dp 圆角。账号设置分组间留白为 12dp，外观页独立面板间距为 8dp；页面边距、导航行前留白与连续列表分隔线保持原值。
...
- [x] “我的”居中展示“概览 / 动态 / 创建 / 参与”四个同级共享点按页签，不再嵌套帖子二级页签；不支持横滑误切，IndexedStack 保留已访问内容。默认概览并行加载创作概览与最近回复，四项统计在两列内居中并可进入对应内容，公开开关不会隐藏本人内容；公开页同样只让可见统计进入动态、创建、参与或回复栏目，“未公开”项不可点击。
