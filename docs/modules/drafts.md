...
提供按账号隔离的本地自动快照、可跨设备列出/继续编辑/删除的未发布服务端主题草稿，以及后端用户级五槽位正文草稿。当前三类草稿均已在创建主题场景形成手动可操作闭环。V1 不做跨设备实时协作、云端自动合并、后台自动发布或正文草稿后台自动保存。

## 2. 用户角色与使用场景

已登录用户创建主题时获得本地防丢快照，可主动保存为未发布主题实体，并从服务端草稿箱继续另一设备保存的完整主题或删除不再需要的条目；也可把当前纯正文保存到五个全局槽位，在另一设备或后续编辑会话中手动恢复。标题、分类、可见范围和标签不进入五槽位正文草稿。

## 3. 页面、入口和导航关系

`/compose/thread` 自动加载本地主题快照并显示本地保存状态。页面顶部“查看服务端主题草稿”打开 90% 高度的完整主题草稿箱，可刷新、继续编辑或二次确认删除；编辑器工具栏的“正文草稿”打开独立五槽位纯正文面板；页面底部“保存到服务端草稿”继续保存当前完整表单。两类服务端草稿拥有不同文案和入口，不互相替代。
...
完整主题草稿箱和五槽位列表都只在当前面板会话内缓存，以服务端列表为事实；完整主题草稿被选中后只把当前一份完整内容写入账号工作快照，其他摘要不进 Drift。两类列表创建、更新或删除成功后失效或更新当前内存状态；关闭面板或身份变化后释放。
