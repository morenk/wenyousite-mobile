...
两类服务端草稿都必须登录。本地快照按 JWT `sub` 分区，但服务端仍是身份和权限事实；切号会释放当前正文草稿状态，不展示其他账号内容。Token、预签名 URL、验证码和密码不得进入草稿。退出登录不静默删除尚未发布的账号快照；账号注销也不会由远端端点删除本机快照，但原账号不可恢复，因此注销前必须把这类草稿视为不可恢复内容并明确提示。
