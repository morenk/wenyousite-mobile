...
`/compose/thread` 自动加载本地主题快照并显示本地保存状态。页面顶部“查看服务端主题草稿”打开 90% 高度的完整主题草稿箱，可刷新、继续编辑或二次确认删除；编辑器工具栏的“正文草稿”打开独立五槽位纯正文面板；页面底部“保存到服务端草稿”继续保存当前完整表单。两类服务端草稿拥有不同文案和入口，不互相替代。`/compose/moment` 与 `/moments/:momentId/edit` 不增加草稿箱入口，而是在重新进入与离页时使用明确对话框处理当前本机草稿。
