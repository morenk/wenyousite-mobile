...
提供按账号隔离的主题本地自动快照、可跨设备列出/继续编辑/删除的未发布服务端主题草稿、后端用户级五槽位正文草稿，以及按账号和新建/动态 ID 分区的本机动态草稿。正文草稿槽位 1 可由当前编辑器显式开启云端自动保存；V1 不做跨设备实时协作、云端自动合并、后台持续保存或自动发布。

## 2. 用户角色与使用场景

已登录用户创建主题时获得本地防丢快照，可主动保存为未发布主题实体，并从服务端草稿箱继续另一设备保存的完整主题或删除不再需要的条目；也可把当前纯正文保存到五个全局槽位，在另一设备或后续编辑会话中恢复。用户可为当前主题、楼层、回复或子贴正文编辑会话开启槽位 1 自动保存，其他四个槽位继续手动管理。标题、分类、可见范围和标签不进入五槽位正文草稿。发布或编辑动态时，标题、正文、封面、已完成图片顺序及其公开派生信息自动保存在本机，重新进入时自动恢复普通草稿并提供撤销入口，离开时确认保留或放弃；待确认创建只允许保留，不能丢弃或重新开始。

## 3. 页面、入口和导航关系

正文草稿自动保存使用全局 Switch 表达开关状态，不重复“已开 / 已关”；保留“自动保存到草稿位 1”、实际等待／保存结果和覆盖风险。无文字的内容显示“暂无文字预览”，不解释格式实现。删除使用统一锚点菜单，仍保留不可恢复确认；恢复与覆盖流程不变。

`/compose/thread` 自动加载本地主题快照并显示本地保存状态。顶栏单一“云端草稿”入口按需提供“保存当前主题”和“打开云端草稿”：前者保存当前完整表单，后者打开 90% 高度的完整主题草稿箱，可刷新、继续编辑或二次确认删除。主题、楼层、回复和子贴正文编辑器工具栏的“正文草稿”打开独立五草稿位纯正文面板；五个位置始终可见，面板顶部提供“自动保存到草稿位 1”开关和“保存到空闲位”快捷动作，每个位置同时保留明确的“保存到这里”入口，已有内容的位置确认后替换，删除收纳在更多操作中。工具栏入口同步显示等待、保存中、已保存或失败状态。两类服务端资源拥有不同文案和入口，不互相替代，也不在页面底部常驻整宽按钮。`/compose/moment` 与 `/moments/:momentId/edit` 不增加草稿箱入口，进入后自动恢复并提示可撤销，离页时用确认对话框处理当前本机草稿；待确认创建不提供撤销或放弃。

设计统一后三批候选／待负责人验收：正文草稿与云端主题草稿复用 showWenyouSheet / WenyouSheetBody，按内容收缩、统一安全区与键盘避让，长列表通过 Sliver 滚动，关闭入口语义一致。用量、失败和操作随内容滚动；取消或系统返回不写入，既有自动保存开启后关闭抽屉继续生效。更新时间复用 WenyouTimeText；恢复、覆盖确认、并发版本与删除规则不变。 交付及手测见[三批统一验收](../architecture/design-unification-completion.md)。
...
组件统一第一批（2026-09-19，负责人验收通过）：快速保存与空草稿位保存接入共享异步按钮；草稿位菜单操作的独立忙碌占位保留，不改变快照写入、覆盖确认和存储行为。 入口清单、自动检查及真机步骤见[组件统一验收](../architecture/component-consistency-acceptance.md)。

选择控件修缮（2026-09-19，负责人验收通过）：正文草稿更多菜单采用统一面板和图标文字排版，删除使用 destructive 语义色；原有二次确认和删除规则不变。 验收清单见[选择控件修缮](../architecture/selection-controls-acceptance.md)。

主题编辑页面把内存 Delta 经类型化块文档安全编码为完整 Markdown，字段变化后 700ms 防抖覆盖主题本地快照；应用暂停、失活或离页前立即刷新。主题本地快照、完整主题草稿和五槽位正文草稿写入都先等待当前粘贴事务并从实时 Delta 重编码；编码失败、重新解析后块结构不等价或 10000 字符超限时保留现有草稿且不发送缓存正文。本地快照不存在远端主题时，首次保存会持久化创建载荷并调用 `threadsCreate`，随后用 `threadsSaveAggregate` 保存当前字段和版本；后续保存直接聚合更新同一未发布主题。发布确认后删除关联本地快照。
...
打开正文草稿面板时用同一服务端快照读取列表和用量。用户可让服务端自动选择下一空位，也可保存到任一指定位置；已有内容的位置必须确认后携带当前版本更新，删除也必须确认。恢复前重新读取单条最新版；当前编辑器已有不同正文时，用户明确选择“替换并恢复”后才替换正文，标题、分类和标签保持不变。服务端版本仅用于并发控制，不作为用户界面信息展示。
...
完整主题草稿箱只在当前面板会话内缓存；五槽位状态以当前编辑器会话为边界，以 `draftsState` 返回的原子快照为事实，关闭面板后仅在自动保存开启时继续存活，编辑器释放或身份变化后释放。完整主题草稿被选中后只把当前一份完整内容写入账号工作快照，其他摘要不进 Drift。两类列表创建、更新或删除成功后失效或更新当前内存状态。

动态草稿复用 Drift v1 现有表，以 `moment:` 加 `(ownerId, momentId)` 的 JSON 元组分区，新建与编辑互不覆盖；保存文字、已完成媒体的主 URL、派生 URL、contentType、animated、尺寸、封面和顺序。500ms 防抖、生命周期保存与删除串行执行。创建前原子保存冻结载荷和 UUID，待确认期间禁止修改或放弃，只能显式重试原操作，不在断网恢复后自动发布。服务端确认成功但本机清理失败时，后续只重试清理，不再次发布、编辑或删除。旧 SharedPreferences `moment.compose.draft.v1:*` 记录缺少可信账号归属，保持原样但不读取、恢复、迁移或自动删除。

## 9. 加载、空数据、错误、重试和冲突状态

反馈简化（2026-09-09 负责人验收通过，源码 `15d4b6b`）：正文草稿保存和删除成功以短 Snackbar 确认，不在面板常驻成功横幅；列表立即反映确认结果。自动保存状态、版本冲突和失败重试继续保留。页面回归检查成功提示退场以及恢复、取消和冲突路径。
...
- [x] 待确认主题创建保存规范化载荷、稳定请求 ID 和确认状态；超时后继续编辑仍先复用原请求确认。动态待确认创建锁定编辑并只重试冻结载荷。
- [x] 发布成功只清理关联本地主题快照。
- [x] 五槽位列表与用量由 `draftsState` 原子读取；创建携带稳定幂等键，删除携带当前版本，相关 operationId 均由仓储测试固定。
- [x] 五个草稿位始终可见；自动选择空闲位、逐位“保存到这里”、版本更新、删除、满额和空正文状态完成，服务端版本不作为界面信息展示。
- [x] 正文草稿允许骰子-only，并在发送前阻止当前正文第 21 个骰子。
- [x] 恢复前读取最新版并确认替换；`40002` 冲突保留本机正文并要求二次确认。
- [x] 五槽位面板覆盖加载、错误、成功反馈、覆盖/删除确认和 360dp 无横向溢出；自动保存固定草稿位 1，其他位置保持可选保存入口。
...
- [x] 动态草稿在 Drift 中按账号和新建/动态 ID 隔离，覆盖重启恢复、防抖保存、恢复/放弃和离页确认；旧无归属草稿保持原样但不恢复。
- [x] 动态待确认载荷与 UUID 原子写入；串行保存/删除不复活草稿，远端确认后的本机清理重试不重复写入服务端。
