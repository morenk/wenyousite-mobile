...
全站搜索的输入框与分类固定在结果列表上方，滚动结果后可直接改词或切换分类。结果仍保持既有惰性请求、分页、失败重试和稳定目标路由；新查询与不同分类使用独立滚动键。
...
组件统一第一批（2026-09-19，负责人验收通过）：动态、全站楼层和主题内楼层结果接入共享分页反馈，保留“加载更多动态／正文／结果”和显式重试；加载失败不丢失结果，不改变查询代次与游标。主题和用户搜索仍沿用现有非分页流程。 入口清单、自动检查及真机步骤见[组件统一验收](../architecture/component-consistency-acceptance.md)。
...
浏览结果中的独立主题、动态、用户和正文卡片遵循 Foundation v7.1.2 的 10dp 圆角与 8dp 卡间距；列表骨架保持同尺寸；独立卡片和连续分隔线列表仍按各自规则排版。
