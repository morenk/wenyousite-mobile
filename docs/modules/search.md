...
组件统一第一批（2026-09-19，候选／待负责人验收）：动态、全站楼层和主题内楼层结果接入共享分页反馈，保留“加载更多动态／正文／结果”和显式重试；加载失败不丢失结果，不改变查询代次与游标。主题和用户搜索仍沿用现有非分页流程。 入口清单、自动检查及真机步骤见[组件统一验收](../architecture/component-consistency-acceptance.md)。
