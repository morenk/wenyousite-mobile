...
详情按作者与完整标题、图片轮播、纯文本正文的顺序呈现，并展示累计加油；标题/正文保留内边距，轮播只保留页面级外边距，360dp 下宽 336dp。正文与评论只把带名称的合法站内坐标，以及裸 `wenyou.site` / `www.wenyou.site` 主题或 16 位私密邀请坐标渲染为可点传送门，普通 Markdown、外部地址、非法邀请和歧义查询保持完整字面文本；主题、子贴、楼层、讨论、回复与邀请坐标均交给应用路由重新校验权限。多图在固定舞台内一次只显示当前一张，横滑切换不同尺寸时不推动正文。非本人动态在顶栏进入 wallet 加油和举报。评论主列表默认最新，评论和楼中楼回复使用留白、左侧层级线与分隔线。详情右下角以不占正文布局高度的紧凑悬浮入口发表评论，列表末尾留出安全空间；顶栏和评论入口始终固定显示，正文与分页评论直接渲染到最终位置，不播放滚动、缩放或入场动画。打开评论后页面入口立即隐藏，普通评论和定向回复只展示唯一的私聊式纯文本 dock，文字、图片、表情、实时字数与发送位于同一紧凑表面，键盘避让空间不绘制成可编辑白区。未发送文字、图片或表情关闭前必须确认，评论最多 500 字，可额外带一张图片或一个收藏表情。详情按来源返回；无来源栈时顶栏和系统返回统一进入动态列表。
...
- [x] 详情评论入口在 360/400/600dp 使用 48dp 内容宽度固定悬浮按钮，不压缩正文视口；滚动、加载和身份变化均不播放顶栏、正文或入口动画。普通评论、定向回复、软键盘避让和未发送离开保护复用唯一的私聊式单体 dock，IME 最终 inset 在首帧直接生效而不跟随键盘动画升降，附件、字数和发送保持在同一紧凑表面，避让区域保持透明且不接入 Markdown/Quill。
...
相册当前一次选择并上传一张，不做并发九图选择；本机草稿不跨设备同步，也不后台自动发布。全屏图库具备规范核心手势，但暂不显示下载/分享入口。动态加油已由 wallet 接入；当前不在信息流卡片直接弹出写入，以详情作为稳定确认上下文。传送门 v1 只做语法识别，不预取目标标题或权限，最终可见性由目标页面服务端请求决定；邀请 token 除正文/评论本身外不写日志、诊断或独立缓存。动态评论通知目前只进入详情；`momentsCommentContext`、`momentCommentNavigation`、目标滚动和短时淡粉边框需作为同一切片接入，不能只拼接一个可能尚未加载的评论 ID。

## 13. 最近审查的契约版本和后端提交

契约 `4.13.1-dev.20260815.1`；Markdown v3（动态正文不消费）；后端 `221e28cd0e25cedb2ad1c7c4c1568175e0c3c9c6`；Foundation `v2.4.2`（`7e7d863`）。
