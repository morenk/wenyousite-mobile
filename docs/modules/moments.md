...
详情按作者与完整标题、图片轮播、纯文本正文的顺序呈现，并展示累计加油；标题/正文保留内边距，轮播只保留页面级外边距，360dp 下宽 336dp。正文与评论只把带名称的站内坐标和裸 `https://wenyou.site/threads/...` 或 `/threads/...` 渲染为可点传送门，普通 Markdown、外部地址和歧义查询保持完整字面文本；主题、子贴、楼层、讨论与回复坐标均交给应用路由重新校验权限。多图在固定舞台内一次只显示当前一张，横滑切换不同尺寸时不推动正文。非本人动态在顶栏进入 wallet 加油和举报。评论主列表默认最新，评论和楼中楼回复使用留白、左侧层级线与分隔线。详情右下角以不占正文布局高度的紧凑悬浮入口发表评论，列表末尾留出安全空间；向下阅读时顶栏和评论入口共同收起，向上轻滑恢复，无障碍导航开启时入口常显。未发送文字、图片或表情关闭前必须确认，评论最多 500 字，可额外带一张图片或一个收藏表情。
...
- [x] 详情评论入口在 360/400/600dp 使用 48dp 内容宽度悬浮按钮，不压缩正文视口；向下阅读时顶栏和入口收起、向上恢复。普通评论、定向回复、软键盘避让和未发送离开保护复用同一底部编辑器。
