...
实现独立于主题帖的短内容动态：发现/关注/用户/收藏双列瀑布信息流、详情、纯文本发布编辑、最多九图、删除、点赞、收藏、两层评论、评论图片、收藏表情与社区举报。当前不把动态正文解析为 Markdown，不做视频、话题、离线发布或本机草稿。
...
发现流按热度读取，关注流按时间读取；发现、关注、收藏和用户信息流以 12dp 间距双列瀑布呈现，卡片只展示封面、两行标题、作者和点赞，正文、评论、收藏与加油进入详情处理。图片封面按服务端宽高预留比例，过长竖图最低收束到 3:4，多图显示数量；文字封面固定 3:4。下拉刷新回到首屏，接近底部或按钮加载下一游标。发布时输入 2～40 字标题、最多 1000 字纯文本，逐张上传最多九张已完成图片，可排序并选择其中一张作信息流封面；无图时由服务端返回确定性文字封面主题。编辑提交当前 `version`，删除需二次确认。

详情按作者与完整标题、图片轮播、纯文本正文的顺序呈现，并展示累计加油；多图在固定舞台内一次只显示当前一张，舞台按封面图（缺失时首图）已知比例预留并收束在 3:4～16:10，横滑切换不同尺寸的 `contain` 图片时不推动下方正文，右下角同步显示当前位置。点按当前图片会从对应序号进入可返回/关闭、双击、捏合、平移、未放大下滑关闭的全屏原图路由。非本人动态在顶栏进入 wallet 加油和举报。评论主列表默认最新，楼中楼默认最早，可按作者筛选；非本人且未删除的根评论与回复可举报。评论最多 500 字，可额外带一张图片，或只带一个收藏表情；图片和表情互斥，回复始终提交真实目标评论 ID。
...
契约 `4.5.0-dev.20260809.1`；Markdown v2（动态正文不消费）；后端 `437e76049a371ff8b6aee1b8a613dc864aa30f11`；Foundation `v1.1.0`（`4974b09a29d5d1c9632f4b2683c8d36c9e3c69bd`）。
