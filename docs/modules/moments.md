...
详情按作者与完整标题、图片轮播、纯文本正文的顺序呈现，并展示累计加油；标题/正文保留内边距，轮播只保留页面级外边距，360dp 下宽 336dp。正文与评论只把带名称的站内坐标和裸 `https://wenyou.site/threads/...` 或 `/threads/...` 渲染为可点传送门，普通 Markdown、外部地址和歧义查询保持完整字面文本；主题、子贴、楼层、讨论与回复坐标均交给应用路由重新校验权限。多图在固定舞台内一次只显示当前一张，横滑切换不同尺寸时不推动正文。非本人动态在顶栏进入 wallet 加油和举报。评论主列表默认最新，评论和楼中楼回复使用留白、左侧层级线与分隔线。详情底部常驻评论入口，未发送文字、图片或表情关闭前必须确认；评论最多 500 字，可额外带一张图片或一个收藏表情。
