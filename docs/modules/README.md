...
所有模块的用户可见排版统一消费 Foundation v7.1.0 的 body/display/utility 语义文字角色并继承平台系统字体；字号、行高、字重、utility 等宽数字特性、功能字体例外与架构门禁见[系统字体迁移验收](../architecture/system-font-migration-acceptance.md)。模块文档只记录业务语境中的层级选择，不维护平行字号表。
