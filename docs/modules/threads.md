...
当前实现公开主题详情、子贴选择、Markdown 正文展示、点赞/收藏/订阅互动、新建主题的本地恢复/服务端草稿/聚合发布，并让楼主或协作者添加和编辑子贴正文、管理已有主题元数据与成员玩家标记。楼主还可任免协作者并在二次确认后删除主题；私密邀请和完整子贴管理后置，V1 不实现举报后台或子贴标签。

## 2. 用户角色与使用场景

游客和登录用户都能阅读公开主题、切换子贴、浏览楼层和查看喜欢数；游客写操作先登录，登录用户可点赞、收藏、创建主题并按发言策略发表楼层。普通登录用户还可订阅官方更新和已标记玩家发言，并可在非楼主时退出自己的玩家身份；楼主/协作者自动收取更新，可维护子贴正文、普通楼层、标题、分区、状态和成员玩家标记。只有楼主可修改可见范围、任免协作者并删除主题；私密邀请属于后续工作台切片。

## 3. 页面、入口和导航关系

首页和搜索主题卡片进入命名路由 `/threads/:threadId`；正文搜索结果使用 `/threads/:threadId?post=:postId` 复用同一详情页，楼层卡片再进入 `/threads/:threadId/posts/:postId/replies`。详情 capability 对管理者开放受保护的 `/threads/:threadId/manage`，其成员入口进入 `/threads/:threadId/manage/members`；保存后返回并刷新详情，删除成功清栈回首页。应用壳创建按钮进入受保护路由 `/compose/thread`，发布确认后替换到新主题详情。游客从互动、发帖、创建或管理入口登录时保留完整目标。子贴切换同时控制正文与楼层数据源；邀请预览为后续二级路由。
...
- 后续工作台：`subthreadsCreate`、`subthreadsUpdate`、`subthreadsRemove`、`subthreadsReorder` 与邀请 operationId。已废弃的 `threadMembersJoin` 不接入；成员身份由服务端发帖等真实业务行为建立。

## 6. 状态模型和数据流

主题详情阶段、详情数据、选中子贴 ID 与当前楼层页分离，控制器丢弃过期请求；目标帖子由独立 FutureProvider 读取。点赞/收藏与订阅分别由 social 的 autoDispose family 管理。创建状态由 editor 模块管理表单、本地快照、远端草稿版本、待确认幂等请求和提交动作。已有主题与成员管理各使用独立 autoDispose family；成员写入串行执行并只替换响应确认的目标成员，退出状态与列表状态分离。身份变化会释放旧状态并切换本地分区。

## 7. 鉴权、权限和隐私规则

公开详情使用 OptionalAuth，游客可读取，登录态附加字段缺失时安全降级。游客点赞或发帖只进入登录，收藏与订阅入口不显示。订阅用 capability 与当前角色识别自动接收更新的楼主/协作者并隐藏控件；同一 capability 只用于显示正文、楼层和主题管理入口，服务端继续复核作者、管理者和发言策略。管理路由本身要求登录，更新、删除与成员管理还要求邮箱已验证；协作者只能改标题、分区、状态与非楼主成员的玩家标记，只有楼主可改可见范围、任免协作者和删除主题。客户端从不允许修改楼主角色；退出只作用于当前用户且楼主无入口。USER 候选仅取已标记普通玩家并排除本人。不可访问主题不泄露标题、成员、正文或缓存内容。

## 8. 本地存储、缓存及失效规则

详情、所选子贴、楼层、互动、订阅、已有主题管理表单和成员管理列表仅做 Riverpod 进程内状态，不提供离线阅读。创建主题把完整 Markdown、表单元数据和稳定创建请求写入 Drift；不保存 Delta，也不静默离线发布。发布确认后删除关联快照；已有主题表单不持久化，未保存变化离页时必须确认放弃。成员写入和退出成功后以服务端响应或详情重读为准，不维护离线成员事实。切号、权限变化、404 或 403 会释放对应会话内状态。

## 9. 加载、空数据、错误、重试和冲突状态

首次加载、重新加载、主题不可访问、无子贴和正文安全降级分别展示明确状态；目标定位另有完整空错重试。互动、订阅、楼层删除、主题管理和成员管理写入串行执行，空响应不伪装成功，失败保留旧状态与请求 ID。成员工作台加载失败可重试，写入失败保留权威旧列表；`40107` 保留页面并可先验证邮箱。创建主题结果不确定时保留原幂等载荷，聚合失败保留本地内容和远端版本。主题元数据收到 `40002`/HTTP 409 时读取最新版但保留本机表单，由用户选择采用云端或基于新 `version` 覆盖。邀请失效状态后续接入。
...
当前完成“详情 + 子贴正文读写 + 楼层读写/楼中楼 + 搜索目标定位 + 点赞收藏订阅 + 创建发布 + 已有主题元数据管理/删除 + 成员身份管理/自行退出玩家”。后续工作台继续完成私密邀请和完整子贴管理。V1 不做举报管理和子贴标签。
