...
阅读时调用主题详情接口加载元数据、标签、登录态互动与 capability 投影；排头只保留一个外层面板：顶部把分类、状态和标签合并为紧凑语境行，标题下用可换行的作者等级、时间、浏览、玩家、楼层与加油元数据代替头像作者块，底部浅色工具栏承载子贴切换和互动，不再为每组按钮套独立卡片。窄屏子贴按钮按可用宽度隐藏目录图标和楼数，仍保留标题、展开入口和所有 48dp 触控目标；600dp 恢复完整信息。选中子贴正文直接铺在阅读画布上，不再套第二层面板；正文基线为约 17sp、1.8 行高、轻微字距，标题层级限制在 1.65/1.35/1.12 倍，引用使用柔和底色与单侧细线，分隔线保持 1px，避免装饰压过长篇文字。楼层使用分隔线形成连续文本流，32dp 紧凑头像、用户名、内容宽度等级和时间保持同一行。每层右侧提供可发现的 48dp 楼层操作入口，零回复也显示“回复”，已有回复显示数量并直接展开预览；预览不重复打印回复总数标题。主题列表和详情标签继续使用行内 `#标签` 链接；只有标签管理工作台使用编辑 Chip。按 `sortOrder` 排列子贴并加载对应正文/楼层，带 `subthread` 目标直接选择有效子贴，带 `post` 目标解析所属子贴并静态定位上下文。主题内搜索要求至少两个 Unicode 字符，按相关度跨全部子贴分页。点赞采用服务端计数，收藏按记录 ID 删除；非楼主可从“更多主题操作”经 wallet 向楼主加油，成功后重读累计值。公开楼层和回复仍可通过长按进入完整举报/编辑/删除菜单。普通登录用户并发读取本人订阅和成员候选；选中有效子贴后，页面底部常驻发表楼层入口并打开 Quill 编辑器。
