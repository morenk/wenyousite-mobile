...
- [x] 首页与标签共享卡片映射只保留安全的作者头像与首张封面；头像缺失或加载失败时使用人物占位，空封面数组不预留图片区，320/360/400/600dp 作者元信息不会溢出。
...
- [x] 楼层连续列表以分隔线代替逐条卡片，点击非交互正文/元信息直接回复且不吞链接、图片、回复数、更多菜单或长按；主题流默认折叠楼中楼正文并只在有回复时保留 48dp 回复数入口，楼层与独立讨论回复保留完整操作菜单。
