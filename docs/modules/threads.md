...
阅读时调用主题详情接口加载元数据、标签、登录态互动与 capability 投影；排头只保留一个外层面板：顶部把分类、状态和标签合并为紧凑语境行，标题下用可换行的作者等级、时间、浏览、玩家、楼层与加油元数据代替头像作者块，底部浅色工具栏承载子贴切换和互动，不再为每组按钮套独立卡片。窄屏子贴按钮按可用宽度隐藏目录图标和楼数，仍保留标题、展开入口和所有 48dp 触控目标；600dp 恢复完整信息。楼层和内嵌回复的等级徽标必须保持内容宽度并紧随用户名，不得因父级松约束拉满列表项。主题列表和详情标签继续使用行内 `#标签` 链接；只有标签管理工作台使用编辑 Chip。按 `sortOrder` 排列子贴并加载对应正文/楼层，带 `subthread` 目标直接选择有效子贴，带 `post` 目标解析所属子贴并静态定位上下文。主题内搜索要求至少两个 Unicode 字符，按相关度跨全部子贴分页。点赞采用服务端计数，收藏按记录 ID 删除；非楼主可从详情顶栏经 wallet 向楼主加油，成功后重读累计值。公开楼层和回复通过长按菜单提供举报/编辑/删除。普通登录用户并发读取本人订阅和成员候选；选中有效子贴后，页面底部常驻发表楼层入口并打开 Quill 编辑器。
