...
- [x] 顶栏不显示“主题详情”，帖子标题在工具栏下方居中；排头使用无 Card 的扁平题头，360/400/600dp 均小于 190dp；最多五标签保持单行横向浏览，子贴切换、喜欢、收藏与订阅保持同一工具栏，窄屏不缩小 48dp 命中区。
...
- [x] Markdown 内合法站内坐标复用 Foundation v5.1 传送门胶囊；门图标、换行不截断、48dp 命中区、内部导航及代码内坐标保持原文有 Widget 回归。
...
- [x] 主题详情在有效子贴下提供按内容收缩的 48dp 固定悬浮发表入口，不扣减正文视口；滚动、加载和身份变化均不播放顶栏、正文或入口动画。游客登录文案、当前子贴目标和底部编辑器打开行为有 Widget 与 360dp 视觉回归。
...
- [x] 子贴目录覆盖列表/单条读取、稳定幂等创建、带版本更新、二次确认删除和完整集合重排；默认子贴保护、冲突刷新及 360/400/600dp 布局通过。
- [x] 主题标签覆盖全局搜索/创建、单条校验、关联列表/添加/移除、五标签上限、同名竞态与失败恢复；公开标签页和 360/400/600dp 布局通过。
