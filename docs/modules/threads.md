...
- [x] 默认子贴选择正确；排头卡片可展开完整目录并用左右按钮切换，任一入口都同步切换 Markdown 正文和对应楼层页。
