...
当前实现公开主题详情、子贴选择、Markdown 正文展示、点赞/收藏/订阅互动、新建主题的本地恢复/服务端草稿/聚合发布，并让楼主或协作者添加和编辑子贴正文。编辑/删除主题元数据、私密邀请和成员工作台后置；V1 不实现举报后台或子贴标签。

## 2. 用户角色与使用场景

游客和登录用户都能阅读公开主题、切换子贴、浏览楼层和查看喜欢数；游客写操作先登录，登录用户可点赞、收藏、创建主题并按发言策略发表楼层。普通登录用户还可订阅官方更新和已标记玩家发言；楼主/协作者自动收取更新，可维护子贴正文并删除普通楼层。成员角色调整、主题元数据管理及私密邀请属于后续工作台切片。

## 3. 页面、入口和导航关系

首页和搜索主题卡片进入命名路由 `/threads/:threadId`；正文搜索结果使用 `/threads/:threadId?post=:postId` 复用同一详情页，楼层卡片再进入 `/threads/:threadId/posts/:postId/replies`。应用壳创建按钮进入受保护路由 `/compose/thread`，发布确认后替换到新主题详情。游客从互动、发帖或创建入口登录时保留完整目标。子贴切换同时控制正文与楼层数据源；工作台、成员页和邀请预览为后续二级路由。
...
公开详情使用 OptionalAuth，游客可读取，登录态附加字段缺失时安全降级。游客点赞或发帖只进入登录，收藏与订阅入口不显示。订阅用 capability 与当前角色识别自动接收更新的楼主/协作者并隐藏控件；同一 capability 只用于显示正文/楼层管理入口，服务端继续复核作者、管理者和发言策略。USER 候选仅取已标记普通玩家并排除本人。不可访问主题不泄露标题、成员、正文或缓存内容。
...
首次加载、重新加载、主题不可访问、无子贴和正文安全降级分别展示明确状态；目标定位另有完整空错重试。互动、订阅和楼层删除串行执行，空响应不伪装成功，失败保留旧状态与请求 ID。创建主题结果不确定时保留原幂等载荷，聚合失败保留本地内容和远端版本。正文和楼层版本冲突由 posts 编辑器处理；邀请失效状态后续接入。
...
当前完成“详情 + 子贴正文读写 + 楼层读写/楼中楼 + 搜索目标定位 + 点赞收藏订阅 + 创建发布”。编辑或删除主题元数据仍后置；后续完成工作台、成员和私密邀请。V1 不做举报管理和子贴标签。
