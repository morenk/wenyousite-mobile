...
- [x] 排头使用无 Card 的扁平题头，360/400/600dp 均小于 190dp；最多五标签保持单行横向浏览，子贴切换、喜欢、收藏与订阅保持同一工具栏，窄屏不缩小 48dp 命中区。
- [x] 共享等级徽标始终按内容宽度收缩；主题主阅读流最多展示前五条楼中楼，单条简短回复不折叠，合计正文超过 500 字符才出现固定高度渐变与展开入口。
- [x] 子贴正文直接位于阅读画布且没有嵌套 Card；楼层作者信息在紧凑单行内完成，整层语义暴露“回复第 N 楼”点击动作；零回复不显示重复按钮，有回复时保留至少 48dp 的回复数入口。
