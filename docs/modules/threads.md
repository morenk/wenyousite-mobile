...
阅读时调用主题详情接口加载元数据、标签、登录态互动与 capability 投影；排头只保留一个外层面板：顶部把分类、状态和标签合并为紧凑语境行，标题下用可换行的作者等级、时间、浏览、玩家、楼层与加油元数据代替头像作者块，底部浅色工具栏承载子贴切换和互动，不再为每组按钮套独立卡片。窄屏子贴按钮按可用宽度隐藏目录图标和楼数，仍保留标题、展开入口和所有 48dp 触控目标；600dp 恢复完整信息。选中子贴正文直接铺在阅读画布上，不再套第二层面板；正文基线为约 17sp、1.8 行高、轻微字距，标题层级限制在 1.65/1.35/1.12 倍，引用使用柔和底色与单侧细线，分隔线保持 1px，避免装饰压过长篇文字。楼层使用分隔线形成连续文本流，32dp 紧凑头像、用户名、内容宽度等级和时间保持同一行。每层右侧提供可发现的 48dp 楼层操作入口，零回复显示“回复”，已有回复只显示弱化数量并进入独立讨论；主题主阅读流不渲染楼中楼作者、等级或正文，避免在接力段落之间插入讨论。主题列表和详情标签继续使用行内 `#标签` 链接；只有标签管理工作台使用编辑 Chip。按 `sortOrder` 排列子贴并加载对应正文/楼层，带 `subthread` 目标直接选择有效子贴，带 `post` 楼层目标解析所属子贴并静态定位，具体回复目标直接打开并精确定位独立讨论。主题内搜索要求至少两个 Unicode 字符，按相关度跨全部子贴分页。点赞采用服务端计数，收藏按记录 ID 删除；非楼主可从“更多主题操作”经 wallet 向楼主加油，成功后重读累计值。公开楼层可通过长按进入完整举报/编辑/删除菜单，回复操作集中在独立讨论页。普通登录用户并发读取本人订阅和成员候选；选中有效子贴后，以不占正文布局高度的紧凑悬浮入口打开 Quill 编辑器，列表末尾保留安全留白。向下阅读时详情顶栏和发表入口共同收起，向上轻滑立即恢复；无障碍导航开启时发表入口不自动隐藏。
...
- [x] 共享等级徽标始终按内容宽度收缩；主题主阅读流默认不渲染楼中楼作者和等级，避免辅助信息挤压接力正文。
...
- [x] 楼层连续列表以分隔线代替逐条卡片，主题流默认折叠楼中楼正文并只保留 48dp 回复数入口；楼层与独立讨论回复保留完整操作菜单。
...
- [x] 主题详情在有效子贴下提供按内容收缩的 48dp 悬浮发表入口，不扣减正文视口；向下滚动进入无顶栏、无悬浮遮挡的专注阅读态，向上恢复。游客登录文案、当前子贴目标和底部编辑器打开行为有 Widget 与 360dp 视觉回归。
