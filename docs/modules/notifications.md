...
组件统一第一批（2026-09-19，候选／待负责人验收）：单条通知删除接入共享异步图标按钮；列表分页继续使用已有共享入口，批量已读由消息中心提供，通知导航和已读事实不变。 入口清单、自动检查及真机步骤见[组件统一验收](../architecture/component-consistency-acceptance.md)。
