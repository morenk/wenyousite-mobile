...
通知目标开头定位候选／待负责人验收：目标页面共享开头对齐，主楼、楼中楼及动态评论均优先完整露出作者信息和正文开头；存在吸顶栏时位于其下方。末尾短内容受实际滚动底部约束并保持整块可见，不新增空白。通知坐标与已读行为不变，详见[定位候选验收](../architecture/floor-start-anchor-acceptance.md)。
