...
首次读取以连续通知行结构 Skeleton 保持列表层级；登录引导、无通知、筛选为空、首屏失败、局部分页失败和目标已失效分开处理。所有 API 错误保留安全用户文案与请求 ID；乐观已读/全部已读失败时回滚并提示，删除失败不移除条目。
...
契约 `5.2.0-dev.20260818.1`；Markdown v3；后端 `534c454bafc64718f5b93d52d66e8888db330dcd`。
