...
楼层跳转过渡候选／待负责人验收：通知打开主楼层或楼中楼回复时，目标页使用共享讨论骨架与定位提示，真实目标布局稳定后按 Foundation 标准动效显现并提示落点。减少动画、失败重试和慢等待返回仍可用；通知目标解析与已读行为不变。见[跳转过渡验收](../architecture/target-transition-acceptance.md)。
