...
楼层跳转过渡（2026-09-27，负责人验收通过）：通知打开主楼层或楼中楼回复时，目标页使用共享讨论骨架与定位提示，真实目标布局稳定后按 Foundation 标准动效显现并提示落点。减少动画、失败重试和慢等待返回仍可用；通知目标解析与已读行为不变。见[跳转过渡验收](../architecture/target-transition-acceptance.md)。
...
选择控件修缮（2026-09-19，负责人验收通过）：通知分类使用统一选择菜单；入口保留安静文字与小箭头，展开时呈现状态反馈，选项右侧勾选，重选当前分类仅关闭菜单。 验收清单见[选择控件修缮](../architecture/selection-controls-acceptance.md)。

通知目标开头定位（2026-09-11，负责人验收通过）：目标页面共享开头对齐，主楼、楼中楼及动态评论均优先完整露出作者信息和正文开头；存在吸顶栏时位于其下方。末尾短内容受实际滚动底部约束并保持整块可见，不新增空白。通知坐标与已读行为不变，详见[定位候选验收](../architecture/floor-start-anchor-acceptance.md)。

进入消息分支、回到前台及每 30 秒刷新通知与私聊未读事实；前台轮询由应用壳单一定时器统一调度。Android 后台提醒默认开启并可在账号设置关闭；服务确认运行后先记录最新一页通知 ID 与内容指纹，随后在进程仍获系统调度时全程每 30 秒尽力拉取且不允许请求重叠。同一聚合通知的 `totalCount` 或摘要变化可再次提醒，历史基线不会补发；只有系统卡片提交成功后才更新内容指纹，接口或展示失败会在下一节拍重试。一次最多显示三条，更多时只显示消息中心汇总。消息中心顶层仍使用“通知 / 私聊”线性页签；通知内部的“全部 / 互动 / 订阅 / 系统”改为右对齐的紧凑筛选菜单，避免连续两层页签压缩列表视口。未读通知条目在尾部显示 8dp Foundation `destructive` 圆点，与普通品牌强调色明确区分；打开通知后乐观标记已读且不阻塞导航，支持确认后单条删除和按服务端全局事实执行全部已读。回复通知根据收件人与真实回复对象显示“回复了你”或“回复了用户名”，订阅者不会被误写为回复对象；缺少新字段的历史通知使用中性“回复了”。动态评论通知只传稳定 momentId/commentId；详情调用 `momentsCommentContext` 注入主评论和目标，不扫描评论分页，并把目标滚动到视口内。下拉刷新会并行校准列表与通知未读数，加载更多失败保留已加载内容。私聊角标由 direct-messages 独立读取，不并入通知筛选或“全部已读”；底栏角标只在展示层求和。

每条通知把操作者、动作和安全摘要合并在最多四行内展示，超出后在第四行末尾省略；旧通知的完整安全正文同样遵循四行上限。进入通知对应的主题、楼层、动态或用户页后，目标页面仍展示完整内容，不受列表摘要行数影响。

通知目标以必填 `target.state` 为权威：只有 `ACTIVE` 允许按 `target.kind` 导航；`CONTENT_DELETED` 根据通知仍保留的评论、动态或主题/楼层坐标显示“该评论已删除”“该动态已删除”或“该内容已删除”，坐标也缺失时显示“该内容已删除或不可访问”；`USER_DEACTIVATED` 显示“该用户已注销”；`NO_TARGET` 按普通无目标系统通知展示，不误报为删除。删除和注销历史态防御性按已读呈现，不重复发送已读请求或后台系统提醒，点按只反馈当前历史态。
...
单条删除的即时列表与角标更新使用现有请求代次保护；切换筛选、刷新或释放后，旧请求不得恢复旧列表，也不得重复扣减未读数。删除期间暂停角标回读，失败先恢复本次扣减，再尝试校准；即使回读断网也不会遗留少算的角标。后续全部已读已清空角标时不恢复旧通知计数，结算只执行一次。删除已于 2026-09-13 获负责人确认通过并授权合并，见 [乐观更新验收](../architecture/optimistic-interactions-acceptance.md)。
...
已读、全部已读和删除均绑定发起时的列表代次；筛选或刷新后到达的成功/失败不得覆盖新快照，旧控制器的确认对话框回调不能再发送请求。分页与写入互斥。乐观角标修改使更早的查询作废，写入结束后强制重读服务端计数，避免旧读取覆盖新结果。

通知未读数是服务端事实；列表与角标共享 `notifications/application` 仓储端口但独立请求，`main.dart` 组合根绑定 API data 适配器。列表控制器按 Foundation 分组隔离游标并复用 `RequestEpoch` 丢弃筛选切换、刷新或首页重载前的迟到响应；加载更多按通知 ID 去重并保持已有顺序，写操作串行化。单条已读和全部已读先乐观更新列表与角标，失败恢复列表并重新请求未读数；确认删除后立即移除条目并递减未读角标，失败按发起列表代次恢复原位置并重读计数。`NotificationTargetState` 保存服务端目标状态，导航先验证 `ACTIVE`，不因仍残留旧坐标而猜测可见性。消息中心与后台轮询组合位于 `app_shell`；通知结构化文案、旧正文清洗和目标坐标是 `notifications/application` 的纯函数，可同时供页面与系统卡片复用，domain 仍只保存服务端事实。后台 poller 以候选批次携带下一份指纹，系统卡片成功后才提交；后台 epoch 在恢复前台时作废并清空基线，迟到响应不得显示或提交。未知枚举保留通用条目，不使整个列表反序列化失败。

## 7. 鉴权、权限和隐私规则

通知授权（2026-09-13，负责人验收通过）：系统授权仅由用户明确点击触发，登录/恢复不重复询问；应用壳一次性提示与账号设置入口负责引导。高重要性 `wenyou_messages_v1` 显式 `playSound: true`，不指定音频资源，由插件选择 `RingtoneManager.TYPE_NOTIFICATION` 的系统默认 URI；既有频道的声音、振动、横幅、锁屏与勿扰选择保持，不删除或换 ID 绕过。小米独立悬浮通知开关不等同于悬浮窗权限，也不能仅凭频道高重要性推断已打开。常驻低重要性频道继续静音。

只允许当前账号读取；切号立即清缓存并停止上一会话的后台尽力检查。系统卡片正文只使用页面同源的安全摘要、合并空白并截断至 160 个 Unicode 字符；不附带完整 Markdown 图片地址。未知 target 不猜测导航，点击回退消息中心。点击新生成的站内系统卡片时，以携带的通知 ID 和接收账号确认单条已读；跳转不等待回执或角标读取。成功后失效已缓存通知列表并强制重读角标，未打开过的列表不额外加载；失败提示并允许明确重试，不先伪造本地已读。重复在途点击合并，切号后迟到结果不更新新账号。旧版无通知 ID 的卡片仅跳转，汇总不全部已读，私聊继续由会话页处理。

## 8. 本地存储、缓存及失效规则

列表与未读数依赖 `ViewerScope`；账号 A 直接切换到账号 B，即使认证布尔值不变也清空私有状态；同账号 Token 刷新不清空。

通知列表、后台基线和内容指纹只做 Riverpod/Dart 进程内状态；消息中心按需构建“通知 / 私聊”，已访问栏目在消息页面组件树内继续挂载，顶层栏目切换不释放通知列表。通知页离开组件树后列表可释放，退出登录后认证状态变化会停止应用壳轮询、清除消息基线并清零角标，但保留设备提醒开关，再次登录重新创建私有状态。进入分支、回前台、前台 30 秒轮询、已开启时的后台固定 30 秒尽力轮询和写操作负责校准。

## 9. 加载、空数据、错误、重试和冲突状态

拉黑或解除后重读通知列表和未读数，不保留此前可见的投影。
...
首次读取以连续通知行结构 Skeleton 保持列表层级；登录引导、无通知、筛选为空、首屏失败、局部分页失败和目标已失效分开处理。加载更多遇到 `40007 INVALID_CURSOR` 时清空当前筛选的旧游标并从首页重载；其他分页失败保留已加载通知、游标和问题编号。动态评论上下文 404 时详情与普通评论继续可读，只提示“目标评论已不可见”且不重试；临时失败提供独立“重试定位”，不重新加载或清空详情。所有 API 错误保留安全用户文案与请求 ID；乐观已读/全部已读失败时回滚并提示，删除失败恢复条目。
...
- [x] 千条通知首屏仅构建视口邻近条目，末项可滚动到达；删除尾项/唯一项不篡改游标，空页仍可继续分页；三类写入迟到成功/失败、释放后入口及角标旧查询有回归。
- [ ] 真机长通知列表检查滚动、删除尾项、筛选切换与角标校准。
- [x] 列表、紧凑分类筛选、分页、单条/全部已读和删除通过仓储、控制器、页面与 360dp Golden 测试。
- [x] 结构化通知与旧正文在列表中统一最多显示四行并省略超出内容，操作者与动作不会脱离摘要单独突破行数限制。
...
- [x] 主楼层、楼中楼回复、thread/user/moment 仅在 `ACTIVE` 时精确导航；删除评论、动态、其他内容、注销用户、普通无目标和未知状态均安全降级，删除/注销历史态不显示未读点或发送后台提醒。
...
V1 仅 API 拉取，不做 FCM、WebSocket、SSE、后台唤醒或后台角标实时保证；Android 默认开启可关闭的设备级后台提醒，使用 specialUse 常驻服务改善执行条件，30 秒 HTTP 检查并非离线推送；负责人已于 2026-09-13 在最终候选确认通过；30 分钟、Doze 等扩展场景没有逐项验收数据。Doze、厂商省电、断网或进程终止可能延迟/停止提醒，重新进入应用时仍由服务端列表校准；不申请电池优化豁免，也不据 mobile push 协议提前注册设备。契约 5.9 的协作者任免通知字段已进入生成客户端，但当前 formatter 与导航尚未识别这两类 action，需与协作主题列表和权限撤销刷新一起接入。
