...
通知授权细节候选（2026-09-13，待负责人验收）：系统授权仅由用户明确点击触发，登录/恢复不重复询问；应用壳一次性提示与账号设置入口负责引导。高重要性 `wenyou_messages_v1` 显式 `playSound: true`，不指定音频资源，由插件选择 `RingtoneManager.TYPE_NOTIFICATION` 的系统默认 URI；既有频道的声音、振动、横幅、锁屏与勿扰选择保持，不删除或换 ID 绕过。小米独立悬浮通知开关不等同于悬浮窗权限，也不能仅凭频道高重要性推断已打开。常驻低重要性频道继续静音。
...
- [ ] 系统卡片点击同步已读候选：冷/热点击、未加载列表、重复点击、失败重试、跨账号、旧载荷及汇总边界有针对性回归；实际通知栏/横幅分别点击后的列表已读与角标变化待负责人复验，见[候选记录](../architecture/notification-read-acceptance.md)。
...
V1 仅 API 拉取，不做 FCM、WebSocket、SSE、后台唤醒或后台角标实时保证；Android 默认开启可关闭的设备级后台提醒，使用 specialUse 常驻服务改善执行条件，30 秒 HTTP 检查并非离线推送；当前候选须负责人真机复验。Doze、厂商省电、断网或进程终止可能延迟/停止提醒，重新进入应用时仍由服务端列表校准；不申请电池优化豁免，也不据 mobile push 协议提前注册设备。契约 5.9 的协作者任免通知字段已进入生成客户端，但当前 formatter 与导航尚未识别这两类 action，需与协作主题列表和权限撤销刷新一起接入。契约 5.11 的通知目标 `state` 已进入生成客户端；服务端会把非 `ACTIVE` 目标降为不可导航的 `kind=none` 并强制已读，移动端尚未按 `CONTENT_DELETED`、`USER_DEACTIVATED` 和 `NO_TARGET` 分别展示历史态文案，该行为需在独立通知切片接入。
