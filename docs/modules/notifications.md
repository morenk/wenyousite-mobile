...
消息中心的“通知 / 私聊”除点按页签外，也支持在内容区左右滑动切换；切换继续写入规范 URL，短滑与首尾越界不改变栏目，私聊 capability 关闭时不会产生非法目标。
...
- [x] 私聊 capability 开启时，消息内容区左右滑动在“通知 / 私聊”间切换并同步规范 URL；能力关闭、短滑和边界滑动均不产生非法切换。
