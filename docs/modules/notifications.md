...
单条删除的即时列表与角标更新使用现有请求代次保护；切换筛选、刷新或释放后，旧请求不得恢复旧列表，也不得重复扣减未读数。删除期间暂停角标回读，失败先恢复本次扣减，再尝试校准；即使回读断网也不会遗留少算的角标。后续全部已读已清空角标时不恢复旧通知计数，结算只执行一次。删除候选待负责人真机验收，见 [乐观更新验收](../architecture/optimistic-interactions-acceptance.md)。
