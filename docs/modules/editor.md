...
- [x] 阅读态与 Quill 编辑态的骰子表达式/结果共用 Foundation v6.2.0 无图标原子节点；紧凑三字段、按需常用面数菜单、标题副行 canonical 预览与 `N/20`、会话内成功值复用、UUID nodeId、当前正文 20 个上限、48dp 命中区、整面板 80% 高度上限、逐骰明细、焦点恢复、混排、Codec 往返和 360dp 键盘视觉基线均有回归。
