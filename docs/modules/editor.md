...
开启正文自动保存前先刷新槽位 1；已有内容时明确确认覆盖风险。启用后完整 Markdown 以 800ms 防抖写入云端，关闭草稿面板后仍跟随当前编辑器，离开编辑器或切号即释放。首次创建携带稳定幂等键，后续更新携带最新版本；空正文不发送，失败或检测到其他设备改动时自动停用并保留可见状态，需用户重新开启。
...
- [x] 主题、楼层、回复和子贴正文编辑器可为槽位 1 开启自动保存；关闭面板后继续防抖写入，状态回显、幂等创建、连续版本更新和冲突停用均有回归。
