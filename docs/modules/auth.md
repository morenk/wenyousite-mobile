...
组件统一第一批（2026-09-19，候选／待负责人验收）：重置密码的验证码发送按钮接入共享描边异步按钮，保留冷却、验证码发送与提交条件；输入、认证和重放规则不变。 入口清单、自动检查及真机步骤见[组件统一验收](../architecture/component-consistency-acceptance.md)。
