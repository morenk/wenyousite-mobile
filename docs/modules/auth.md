...
组件统一第一批（2026-09-19，负责人验收通过）：重置密码的验证码发送按钮接入共享描边异步按钮，保留冷却、验证码发送与提交条件；输入、认证和重放规则不变。 入口清单、自动检查及真机步骤见[组件统一验收](../architecture/component-consistency-acceptance.md)。
...
2026-09-23 契约同步复核：Backend `92b030a81f8957386e324fed477bd1e46faf65ea`、OpenAPI `5.26.0-dev.20260922.3`，与公网 `/meta` 一致；管理端会话字段不适用于移动端，本模块既有行为不变。
