...
组件统一第一批（2026-09-19，候选／待负责人验收）：举报提交使用共享异步按钮，统一宽度保持、加载反馈和禁用行为；原因／说明表单及在途取消、返回和外部关闭保护保持不变。 入口清单、自动检查及真机步骤见[组件统一验收](../architecture/component-consistency-acceptance.md)。
