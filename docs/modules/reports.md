...
选择控件修缮候选（2026-09-19，待负责人验收）：举报原因保持明确字段标签，使用与表单协调的描边入口和共享选择菜单；选项自然换行、右侧勾选，提交在途仍禁用，补充说明的必填规则不变。 验收清单见[选择控件修缮](../architecture/selection-controls-acceptance.md)。
