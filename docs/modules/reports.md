...
组件统一第一批（2026-09-19，负责人验收通过）：举报提交使用共享异步按钮，统一宽度保持、加载反馈和禁用行为；原因／说明表单及在途取消、返回和外部关闭保护保持不变。 入口清单、自动检查及真机步骤见[组件统一验收](../architecture/component-consistency-acceptance.md)。

选择控件修缮（2026-09-19，负责人验收通过）：举报原因保持明确字段标签，使用与表单协调的描边入口和共享选择菜单；选项自然换行、右侧勾选，提交在途仍禁用，补充说明的必填规则不变。 验收清单见[选择控件修缮](../architecture/selection-controls-acceptance.md)。
...
举报表单由打开入口传入固定登录会话，首帧构建前切号也拒绝旧上下文；切号立即撤下自己的弹层，旧提交回调和迟到结果不能归入新账号。若上层已打开其他路由，只关闭举报所属路由。控制器按会话重建，异步结果不跨账号展示。
...
- [x] 切号关闭旧表单、旧回调禁止提交、在途迟到成功不污染新账号及覆盖路由保留的 Widget 回归。
- [ ] 使用身份已验证的独立隔离环境完成六类目标、重复举报和限流联调；公网自动化只读。
...
2026-09-23 契约同步复核：Backend `92b030a81f8957386e324fed477bd1e46faf65ea`、OpenAPI `5.26.0-dev.20260922.3`，与公网 `/meta` 一致；新增管理内容整理接口不适用于移动端，本模块既有举报行为不变。
