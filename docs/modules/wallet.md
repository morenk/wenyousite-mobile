...
app-shell 只负责会话就绪后的签到触发与非阻断提示；wallet 通过 app 组合层的 `ProfileCacheInvalidator` 发布失效意图，不直接导入 users provider。users、threads 和 moments 只挂载目标入口并在成功后重读自身服务端投影。钱包流水目标导航复用这些模块的稳定路由。所有 UI 只消费 Foundation v1.2.1 Token、全局主题和共享面板；精确金额字符串是 Wallet 与动态/主题/用户累计加油的共同约束。
