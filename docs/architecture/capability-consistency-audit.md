...
## 2026-09-23 合并最新 dev 与契约复核

负责人明确授权 PR #51 合并。先普通 merge `origin/dev` 的 `78224480bcb95aa0af88b06820fcfa7f671c936d`，保留双方有效行为：正文草稿采用共享 Sliver 抽屉但继续按码点判断10000上限；回复时间通过 `WenyouTimeText` 输出日期语义，不重复拼接；单图短提示、简介码点计数、凭证边界和互动权限保持。双方历史模块／验收记录均保留。

首轮完整门禁在公网契约检查停止：部署已为 Backend `92b030a81f8957386e324fed477bd1e46faf65ea`／API `5.26.0-dev.20260922.3`，原5.24来源不再是运行版本。没有关闭或放宽门禁；复用另一任务已提交且不含通知业务的契约提交 `f05e35e1`、`3e916484`、`f16db448`，在本分支保持独立 chore／docs 提交并解决旧来源记录冲突。重新生成客户端验证同一快照，诊断路由同样用正式生成器补齐。5.26首次重跑在生成阶段为补齐诊断索引主动停止，不记为通过；最终源码冻结后重新执行完整门禁。

原223操作审计是固定5.24的历史证据，不改写为已验证225。当前5.26共225操作：原158项调用适用性保持，排除登记由65增为67；新增两项均为“待验证／未接入”，不是平台差异或行为通过。

| 新增操作 | 契约依据与范围 | 本次状态 |
| --- | --- | --- |
| `usersFollowRemoveFollower` | 5.25兼容新增本人移除粉丝；本人列表新增可选双向关系投影 | 待验证；本PR不接入入口，独立关系管理切片负责 |
| `galleryList` | 5.26兼容新增图集OptionalAuth查询、双向游标与40926索引未就绪 | 待验证；本PR不接入连续图集，独立媒体切片负责 |

5.25管理记住设备与管理内容字段由管理端消费，不扩大Mobile产品范围。既有Markdown、上传完成态及媒体账本语义不变。没有线上自动化写入；完整门禁的公网请求仅只读契约与公开读取，仍未关闭Sentry或代替真机验收。合并成功后，按治理与产品规则保留本地／远端任务分支和Worktree，原因是原问题仍待负责人真机验收。

本轮生成缓存出现无进度时只停止本工作树进程，将本工作树旧缓存保留为 `packages/wenyou_api/.dart_tool/build-before-capability-merge-20260923` 后重新生成，未改生成源码或其他任务环境。随后一轮在文档门禁发现 notifications 缺少5.26来源记录；仅独立补齐来源、保留未接入说明后，通过统一入口重跑。以上停止轮均保留日志，不计为成功。

最终冻结输入执行 `npm run check:apk -- -TestConcurrency 2` 退出0：OpenAPI校验、固定来源、API重生成无漂移、线上只读契约／Markdown兼容性、1050文件格式零变更、应用与生成包静态分析、架构、21模块文档、158适用接口覆盖全部通过；Flutter **4693通过、1项显式真实Sentry收件跳过、0失败**，Windows工具 **18通过**，Android Debug APK构建通过。唯一最终成功日志为 `build/capability-merge-final-check-complete.log`。应用源码冻结后未再修改，不重复门禁。

新包核实为 `site.wenyou.app.debug`、`0.7.1-debug`／95、最低API26／目标API36，223819658字节，SHA-256 `3A49EE712E5CB7D060E4DD9701AB2DB0694F58D72EE22EF0BB103D6C758C4A95`。独立归档目录 `D:/code/wenyousite/artifacts/mobile-capability-merge-20260923/` 中的 `candidate.json` 绑定最终源码提交、合并提交、APK与日志；`application-inputs.json` 固定1667项输入，聚合指纹 `a9e9bcee3073b5bad3bf04c018ac62d4e93043c053448776957305111d8fdec0`，按前述算法并在提交后核验不变。旧 d3 包不包含本轮 dev／5.26整合，仅保留历史。未安装、发布或部署；原问题仍待负责人真机验收。
