...
2026-09-20 负责人已要求开始后续统一，六类分歧按四批推进，第一批处理设置行和选择项。执行范围、顺序与验收见[六类设计统一计划](design-unification-plan.md)。以下保留排查时的原始位置；第一批将移除 `_ThreadSettingRow` 和 `_AppearanceOption`。
