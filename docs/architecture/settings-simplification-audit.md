...
2026-09-20 负责人已要求开始后续统一，六类分歧按四批推进。第一批已移除 `_ThreadSettingRow` 和 `_AppearanceOption`；后三批随后按负责人要求一同实现，见[后三批候选验收](design-unification-completion.md)。执行范围、顺序与验收见[六类设计统一计划](design-unification-plan.md)。以下保留排查时的原始位置与建议，候选均待负责人真机验收。

以下源码分歧未在原副标题清理候选中迁移，现已由上述后续候选覆盖；不是新增一份与 Foundation 并行的视觉规范。
