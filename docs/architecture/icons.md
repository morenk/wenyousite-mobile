...
移动端图标以 Foundation v6.10.0 的语义注册表为唯一事实源。生产代码只通过 `WenyouIcon` 与 `WenyouIconIds` 表达“回复、加油、本人资料、发布”等业务含义，不直接引用 Flutter `Icons.*`、`IconData` 或 `Icon(...)`。这样 Web 与移动端会从同一语义名称得到一致图形，例如回复统一为对话气泡，加油统一为加油站，而不再由各页面自行选择左箭头、火苗等近似图标。
...
2026-09-11 升级固定正式 Tag `v6.10.0`（`72f28500aee561cde3036a27699b5fc864bbe32d`）。该版本兼容新增阅读快翻语义，既有图标和数值 Token 保持；一并发布的动态图与块边界规范已由最新 dev 的对应实现承接，随本次完整消费者回归复核。按钮接入在后续独立行为提交中完成。
