...
- 本人资料在统计数字下方、昵称右侧提供小号描边“编辑资料”按钮，账号设置“账号”首项保留同一入口，两者复用 `/me/edit`。本人公开页使用相同按钮；其他用户不显示编辑入口。头像嵌入封面的部分不再额外占位，昵称靠近头像；签名独立左对齐，短经验条与经验数值在签名下方同一行显示。
...
1. 有封面、无封面、长昵称、长签名及两倍字号：封面与进度保留，头像与昵称靠近，短经验条和数值同一行，签名保留独立间距。统计下方小号编辑按钮和设置“账号”首项均进入原资料页并可返回；本人公开页使用相同小按钮，他人公开页不显示编辑入口，工具均可点按。
