...
- 本人主页和公开页撤下编辑按钮，资料修改统一从账号设置“账号”首项进入原 `/me/edit`。头像嵌入封面的部分不再额外占位，昵称靠近头像；签名独立左对齐，经验条和数值共同占满下方一行，数值靠右，进度条填满剩余宽度。
- 本人统计为关注、粉丝和温油。温油读取现有钱包余额；收藏夹、表情包、油卡使用同宽同底色的平面工具栏承接资料区，移除外框和上下灰色间隔，仅底部细线区分内容标签。油卡仍进入原账务路由，入口下不重复余额。公开他人主页不读取私有余额，继续展示公开收到加油。
...
负责人已指出首轮真实封面页面布局不合格，重点是编辑按钮、签名与资料区分组。随后明确要求封面、进度条必须保留，不能用删功能代替重排；实现已恢复并按此约束继续验证。负责人曾要求编辑资料完全收纳到设置列表，后续明确改为统计数字下方的小号按钮，当前按最新要求恢复紧凑入口并复用原修改资料页。未将此前画面记录为验收通过。
...
负责人随后在本人资料区截图指出昵称离头像过远，并圈出昵称、签名右侧的大片空白。复现路径为“我的”，使用有封面、短昵称与短签名的资料：头像绘制向上移动 24dp，但旧布局仍保留完整 72dp 高度，产生 32dp 的头像底部至昵称间隔；经验数值还单独占一行。该轮移除重叠部分的空占位、收紧统计上下间距，并试排经验数值到昵称右侧。负责人进一步选择短经验条与数值同行，最终按该要求排列；普通字号经验条最长 96dp，窄屏大字号允许收短，签名保持独立间距。本批次仍为候选，未记录负责人通过。

后续负责人撤回小号编辑按钮，并明确经验条与数值需要共同填满整行；当前采用数值右对齐、进度条弹性铺满剩余宽度的布局，设置编辑入口保留。工具卡截图同时指出圆角卡片与上下栏目衔接突兀；当前改为同底色平面工具栏，保持等宽入口与原导航，只用底部细线衔接内容标签。此前小按钮、固定短条和圆角工具卡均不作为最终效果。
...
- 22:59 在同一会话热重载小号编辑按钮与短经验条，返回 ready，实际加载源码摘要为 `dcb7014e1b0fb88b2216987daf285e6ff7e24b31dcaf2cb514610e49ca5528d0`；该画面不能用 22:48 初装 APK 的哈希代表。
- 整行经验与平面工具栏调整时原 Flutter 应用已停止，设备仍在线；23:13 重新启动同一批次后 ready，实际加载源码摘要 `e418521c789791fbd3b45dfd6c094eccb43823131581b8795f3d5d9e4404aa63`。Debug 包更新时间 `2026-09-26 23:13:21`，设备与本地 APK SHA-256 一致为 `7ee0712280b7eb6d9cbf78057b712a75d1073194b0589ad19cd83d0b5fad8c2b`；这是恢复开发反馈会话的重建，不是最终质量门禁候选，负责人视觉验收仍待反馈。

## 2026-09-27 预览断连排查与恢复

负责人反馈“个人资料加载失败，问题环境：暂无法判断”。现场检查确认手机在线、App 进程仍在，但上一 Debug 控制器已因 Flutter 会话停止而结束，本批次 Windows API／媒体隧道与手机 ADB reverse 均不存在；经 SSH 只读核对，VPS 的 backend 和 media 身份端点正常且仍属于原 runId。资料请求在预览身份探测阶段被阻止，兜底异常分类显示“暂无法判断”；未取得足够证据确定更早的 Flutter 连接中断由何触发，不将其推断为资料丢失或布局故障。

在设备 `4b9c39b5` 恢复同一 `real-history-preview` 会话后，00:46 返回 ready，API／媒体身份核验通过、两条手机端口映射恢复；实际加载源码 `3626dd1765418a3211763ff6a9ee00388228cc3f`，设备与本地 APK SHA-256 均为 `7ee0712280b7eb6d9cbf78057b712a75d1073194b0589ad19cd83d0b5fad8c2b`。随后状态复查仍 ready。负责人按“我的 → 重试”复核并明确回复“已恢复正常”，本次资料加载失败的环境恢复已验收通过；未修改应用代码，不代替整个视觉批次或预览工具的最终门禁验收。
...
小号编辑按钮与短经验条补充：同上五个测试文件首轮 88 项通过，公开本人页的两项检查发现搬走底部编辑入口后仍创建空操作栏；改为空时不渲染后，单独复跑 `test/features/users/public_user_page_test.dart` 全部 17 项通过，覆盖全部 90 项相关用例。新增主页按钮进入既有编辑页并返回，保留设置入口同样路径；视觉检查断言按钮处于统计下方、普通字号表面高 32dp／触控高至少 48dp，经验条宽不超过 96dp 并与数值同线。6 个改动 Dart 文件分析零问题，修正公开页后单独分析仍零问题；明暗主页及窄屏两倍字号 Golden 已更新，目视复核普通亮色和两倍字号主页。本轮纯 Dart 反馈使用热重载，未重复构建独立候选 APK。

整行经验与平面工具栏补充：撤回按钮与调整经验行后，上述五个测试文件共 89 项通过；随后工具区衔接调整复跑 `test/features/users/me_page_visual_test.dart`、`test/features/users/me_page_test.dart` 共 54 项通过。更新断言确认经验条左边缘与正文对齐、数值右边缘与正文对齐、两者保持 8dp 间距；工具栏与资料区同宽并首尾相接，下方内容标签无额外断层，吸顶与原设置编辑跳转保持。`lib/features/users/presentation` 和 `test/features/users` 全部静态分析零问题，目视复核 360dp 明暗及 320dp 两倍字号 Golden。本次属于同一开发反馈批次，尚待负责人视觉验收。
