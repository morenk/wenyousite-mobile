...
每个普通请求首次发送时固定会话 generation，成功响应、鉴权错误、刷新和延迟重试均检查该归属；退出或切号后的旧请求仅结束客户端处理，不得换用新账号 Token、清除新会话或写回旧投影。取消不代表服务端写入已撤销，业务待确认操作仍由原账号保留。公开请求移除 Authorization，专用申诉凭据使用 `explicitCredentials` 保留显式头；两者均不刷新或失效普通会话。

安全存储读、写、清理统一串行；登录开始和结束、退出或失效同步推进隔离边界，存储完成后再次核对操作代次。刷新只在同一会话共享 Future，不推进会话代次；旧刷新写入完成后也不得发布凭据。清理失败立即撤销内存凭据并向调用方报告本地失败，后续清理或登录仍可进入存储队列。
...
公共 API、刷新与媒体直传纳入正式包脱敏诊断；界面分级不再决定是否记录，帖子编辑生成内容后被参数/图片/格式校验拒绝也可发送。记录与发送策略、隐私白名单、开关和 requestId 关联见[故障诊断](failure-diagnostics.md)。专用 Debug 布局诊断继续独立启用，不并入远程记录。
