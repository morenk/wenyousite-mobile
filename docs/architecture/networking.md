...
Cursor 是不透明字符串。筛选变化清空 cursor；`40007` 清空列表并从第一页重载。自动重试只覆盖 GET、明确幂等方法以及携带稳定 `clientRequestId` 的创建请求。温油加油同样是幂等写入：一次弹窗操作以规范化目标、金额和 UUID 固定请求，失败后相同金额重试不得轮换 UUID。
