...
`ApiFailure` 只保存可稳定分支的失败原因、恢复动作、HTTP 状态、业务错误码、请求 ID、响应契约版本和重试信息，不把后端 `message` 或底层异常文本当作界面文案。application/presentation 在具体对象上映射为 `UserFacingFailure`，明确“什么失败”、内容是否保留、安全恢复动作和页内/局部/瞬时的展示位置。core 只解释断网、超时、认证失效、频率、分页和通用冲突等横切语义；私聊、动态、贴纸、钱包等业务码仍由各 feature 在 data 边界映射，domain 不依赖网络错误类型。程序逻辑按业务码或 `FailureReason` 分支，未知响应与序列化失败降级为通用失败，仅次要显示“问题编号”供排障。
