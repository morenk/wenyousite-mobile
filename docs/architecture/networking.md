...
可读取权威投影的写操作统一使用 `WriteReconciler`，结果分为已完成、明确失败、正在确认和暂时无法确定。写入遇到超时、断线、瞬时异常或业务层明确列出的可收敛冲突时，只读取一次最新投影：投影证明目标已达成才按完成处理；读取到相反状态或读取本身失败都保持“暂时无法确定”，因为原写入仍可能稍后落地。业务层必须逐项提供目标判定和允许收敛的业务码，通用层不把任意冲突视为成功。确认期间禁止重复点击，页面销毁和旧请求世代的结果不得覆盖新状态。

结果暂时无法确定时使用中性提示和动作化文案，只提供“刷新查看”等安全入口，不自动重发普通写请求；问题编号作为次要信息。明确的权限、参数和内容冲突仍直接失败，例如 `40912` 不进入成功收敛。订阅创建的 `40904` 只有在重新读取后能以 `threadId + type + targetUserId` 精确匹配记录时才视为完成。
