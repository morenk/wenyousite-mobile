...
| 列表/说明正文 | 14sp / 400 / 1.45 | 卡片摘要、说明和辅助内容 |
| 详情阅读正文 | 17sp / 400 / 1.75–1.85 | 主题正文和楼层 Markdown |
| 嵌套回复正文 | 16sp / 400 / 1.75 | 内嵌楼中楼与紧凑讨论内容 |
...
- 详情阅读列在手机保持 16–20dp 正文留白，600dp 及以上限制最大宽度；长链接、连续英文、表格和代码块只能在自身容器换行或横向滚动，不能撑宽整页。
- 系统文字缩放到 200% 时允许详情、楼层和内嵌回复增高与换行，不裁切正文、标签、错误或主要操作。
...
- 详情顶部使用单层主面板组织标题、作者、分类、状态、标签和统计；不把每个元数据拆成独立卡片，也不使用大面积品牌粉背景。
- 子贴选择是详情阅读上下文，使用可横向滚动或换行的弱面板选中控件；选中态同时使用品牌图标/文字和弱粉背景，触控区域至少 48dp。
- 子贴正文保持连续阅读列，使用约 17sp、1.75–1.85 行高；Markdown 标题、引用、代码、链接、图片、提及、骰子和未知节点不能破坏正文节奏或页面宽度。
- 每个楼层可以使用一个白色面板表达作者、楼层号、时间、正文和回复计数；楼层内部不再为每段正文叠加完整面板。
- 内嵌回复放在楼层的弱面板区域，以 16sp、约 1.75 行高形成第二层视觉关系；只允许两层视觉缩进，不按回复链继续向内嵌套。
- 加载更多、到底、空楼层和分页错误属于列表尾部状态，不能覆盖已经加载的正文；尚未实现的“查看全部回复”、回复或社交操作不得显示伪按钮。
...
| 主题详情 | 单列阅读面板 + 明确子贴上下文 + 17sp 舒展正文 | 复制桌面侧栏、正文 14sp 过密或每段套卡片 |
| 楼层与回复 | 一层楼层面板 + 弱面板内嵌回复，最多两层视觉关系 | 无限缩进、每条回复粗边框或未实现操作伪入口 |
