...
- Foundation v6.8.0 的三种对齐语义图标已作为三个独立普通图标按钮接入主题与帖子编辑器“更多”面板，不使用共享胶囊或分段描边；Markdown v4 capability 开启时可直接选择左、居中或右，混合选区不伪造选中项，不可对齐块禁用三个入口。阅读态对普通段落和 H2/H3 应用同一布局，列表、引用、普通图片与分隔线保持默认布局。
