...
- Foundation v6.8.0 的三种对齐语义图标已作为三个独立普通图标按钮接入主题与帖子编辑器“更多”面板，不使用共享胶囊或分段描边；主栏与托盘格式选中态沿用透明静止容器，仅把图标切换为亮/黑夜鲜粉色。Markdown v4 capability 开启时可直接选择左、居中或右，第一次段尾回车后的空行光标与输入继承同一方向，在空方向行再次回车则补出空段并退出到居左，混合选区不伪造选中项，不可对齐块禁用三个入口。阅读态对普通段落和 H2/H3 应用同一布局，列表、引用、普通图片与分隔线保持默认布局。
...
Foundation v6.8.0 Flutter 包已导出对齐能力与左/中/右语义图标，以及骰子主文案、明细、插入器、行高、内边距，引用的颜色、侧线、圆角和排版，分隔线尺寸与主题标签常量。当前对齐入口直接消费语义图标，具体存储、块白名单和剪贴板规则由 Markdown v4/clipboard v2 契约负责；分隔线与主题标签继续通过主题适配层解析亮色/黑夜语义色。提及与行内代码仍有部分数字只存在于机器合同，本仓库共享元素组件暂按同一 Tag 精确消费；上游补齐导出后应迁移为直接引用，避免重复数字继续存在。
