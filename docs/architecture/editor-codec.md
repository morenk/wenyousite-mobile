...
17. 对齐标记只允许紧邻有可见内容且不含普通图片的普通段落或 H2/H3；同一 Markdown 段落的物理行必须保持一致。段尾回车和随后文字若由 Quill 分批提交，回车产生的尾部空行必须先继承前一物理行的合法居中或居右属性，临时属性在空段编码时移除，再进入混合值清洗。列表、引用、普通图片、分隔线、空段、兼容文本和未知标记不得继承或创建对齐属性。
...
- 对齐回归覆盖精确/畸形 marker、代码保护区与兼容文本拒绝、左 → 居中 → 右循环、选择边界与混合块清洗、撤销/重做、标题转换保持、列表/引用清除、普通图片与空块拒绝、多物理行一致性、段尾回车与后续输入分批到达、整块复制/剪切、段内结构粘贴、320～600dp 与 1×/2× 字号阅读分段、clipboard v1 降级与 v2 往返。
