...
服务端声明版本的完整 Markdown 是唯一持久化格式，Flutter Quill Delta 只存在于编辑会话内存。客户端兼容 v3/v4/v5；打开编辑器时由自研 Codec 按 `Markdown → MarkdownEditorDocument → Delta` 将完整快照解析为编辑状态；自动快照、云草稿、显式保存和提交前统一按 `Delta → MarkdownEditorDocument → Markdown` 重新序列化，再执行当前版本的规范化、块结构等价与可见性检查。独立正文预览不是该链路的一部分，Quill 编辑态本身按阅读态等价规格呈现。
...
- 工具栏格式策略覆盖全部可创建命令、15 种兼容叠加格式、独立行内代码及空格分别归属前后格式的 2×16×16 个相邻组合、行内代码互斥和块格式转换；同一份 Markdown 在编辑态与发布态共享样式规格，并维护亮暗色、宽屏及 2× 字号配对 Golden。
- 对齐回归覆盖精确/畸形 marker、代码保护区与兼容文本拒绝、左 → 居中 → 右循环、选择边界与混合块清洗、撤销/重做、标题转换保持、列表/引用清除、v4 普通图片拒绝、v5 独立图片块允许及图片混排拒绝、空块拒绝、多物理行一致性、第一次段尾回车继承、空方向行第二次回车退出及后续输入、整块复制/剪切、段内结构粘贴、320～600dp 与 1×/2× 字号阅读分段、clipboard v1 降级与 v2 往返。
