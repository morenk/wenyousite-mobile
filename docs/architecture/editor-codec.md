...
行内组合候选增加 `MarkdownInlineRuns`：按逻辑样式区间归并，共有 marks 穿过内部空格，代码内部字符不转义；写出前以独立阅读 AST 校验文字及 marks，再保留完整文档的重开语义门禁。新版独立组合语料不修改既有 v7 fixture，来源摘要单独固定；原问题真机状态见[候选验收](editor-inline-combination-acceptance.md)。
...
- 工具栏格式策略覆盖全部可创建命令、15 种兼容叠加格式、独立行内代码及空格分别归属前后格式的 2×16×16 个相邻组合、32 种代码/强调/链接组合和块格式转换；同一份 Markdown 在编辑态与发布态共享样式规格，并维护亮暗色、宽屏及 2× 字号配对 Golden。
