...
16. `* / _ / ~~` 在标点或符号边界紧邻正文时必须恢复既有强调语义；写回仅用数字字符引用保护相邻字符，不增加可见空格，下划线别名统一规范为星号。转义、代码、空白边界和普通单词内下划线保持字面文本。
17. 对齐标记只允许紧邻有可见内容且不含普通图片的普通段落或 H2/H3；同一 Markdown 段落的物理行必须保持一致。列表、引用、普通图片、分隔线、空段、兼容文本和未知标记不得继承或创建对齐属性。
