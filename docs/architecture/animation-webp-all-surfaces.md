...
响应展示描述损坏、源映射重复或媒体身份错配统一记录内容契约错误，展示文字由 `UserFacingFailure` 映射；不支持续查保留本机错误与重新打开动作。处理中是可继续查询的状态，提示在应用／展示层选择。上传完成的 `display.url` 复用既有安全地址规则，公网 HTTP 拒绝，HTTPS 及既有 localhost／回环／模拟器调试例外保留。

冻结源码 `8e442e9dc844c463063745daf80bb48050f24030` 完整门禁首次运行有 4374 项通过、1 项既有跳过和 1 项文案构造数量失败；其余检查含公网 API 精确 SHA 与 17 项工具测试通过。后续小范围修复保留该失败日志，不调整基线，单独记录完整文案测试、受影响失败映射／媒体／私聊／展示回归、分析、架构和文档检查的结果。最终 Debug 对应修复后的精确提交，不把首次全量结果表述为最终 SHA 的全量通过。
