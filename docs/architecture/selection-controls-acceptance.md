...
- 集成全量回归发现 `test/core/widgets/wenyou_discussion_controls_test.dart` 的旧断言仍查找作者 `PopupMenuItem`；已更新为底部抽屉位置、宽度与长名字不截断的断言，该文件 6 项回归通过。仅补齐测试，未改变已验收应用源码。
- 首轮集成全量回归为 4,494 项通过、1 项既有跳过、6 项失败：作者抽屉旧断言 1 项、图片菜单旧 ListTile 查找 1 项、子贴标题调整影响快翻页截图 4 项。图片收藏的 3 项、快翻的 6 项补充回归均通过；四张截图差异已目视确认局限于子贴导航标题与箭头呈现。修正只涉及测试与基线，最终重新运行完整门禁。
- 最终完整门禁 `npm run check -- -TestConcurrency 3` 已通过：4,500 项 Flutter 测试通过、1 项既有跳过、18 项 Windows 发布工具测试通过；格式、全量静态分析、架构、文档、API 覆盖、来源与线上兼容性、客户端再生成一致性均通过。验证提交为 `94f9efc15fb25d13e8d022405f9eb307afee7721`，与已验收 `a7caeb58` 的应用源码及依赖一致；后续仅补充本验收结果。日志保存在 `build/candidates/subthread-navigation-ce9c4ae8/integration-check.log`。
